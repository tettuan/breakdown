    ...
    if (resolverResult.ok) {
      ...
      if (!pathResult.ok) {
        ...
        logger.debug("Expected error", {
          ...
          attemptedPaths: pathResult.error.kind === "TemplateNotFound" ? pathResult.error.attempted : undefined,
        });
      }
    }
  } finally {
  ...
  try {
    ...
    if (resolver1Result.ok) {
      ...
      if (path1Result.ok) {
        ...
        assertEquals(path1Result.data.metadata.baseDir, join(originalCwd, "tests", "tmp", "prompts"));
      }
    }
    ...
    if (resolver2Result.ok) {
      ...
      if (path2Result.ok) {
        ...
        assertEquals(path2Result.data.metadata.baseDir, join(originalCwd, ".agent", "breakdown", "prompts"));
      }
    }
    ...
    for (const struct of structures) {
      ...
      if (resolverResult.ok) {
        ...
        if (pathResult.ok) {
          ...
          assertEquals(pathResult.data.metadata.baseDir, struct.expectedBasePath, `Base dir mismatch for ${struct.name}`);
