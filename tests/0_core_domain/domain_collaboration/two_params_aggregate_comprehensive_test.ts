  ...
  describe("Aggregate creation and validation", () => {
    it("should create TwoParams with all valid directive and layer combinations", () => {
      ...
      for (const combo of validCombinations) {
        ...
        if (result.ok) {
          ...
          assert(
            validationResult.ok,
            `Validation should succeed for ${combo.directive}/${combo.layer}`,
          );
        }
      }
    ...
    it("should handle edge cases in TwoParams creation", () => {
      ...
      const edgeCases = [
        ...
        {
          directive: "very-long-directive-name-that-exceeds-limit",
          layer: "issue",
          expectError: "InvalidDirective",
        },
        {
          directive: "to",
          layer: "very-long-layer-name-that-exceeds-the-maximum-allowed-length",
          expectError: "InvalidLayer",
        },
  ...
  describe("Path resolution comprehensive testing", () => {
    ...
    it("should handle adaptation parameters comprehensively", () => {
      ...
      for (const test of fromLayerTests) {
        ...
        logger.debug(
          `Test fromLayer: ${test.fromLayer}, expected: ${test.expected}, got: ${resolvedPath}`,
        );
  ...
  describe("Equality and comparison operations", () => {
    it("should implement correct equality semantics", () => {
      ...
      assert(
        ...
        "All TwoParams creation should succeed",
      );
      ...
      assert(
        twoParams1a.directive.equals(twoParams1b.directive),
        "Same directives should be equal",
      );
      ...
      assertFalse(
        twoParams1a.directive.equals(twoParams2.directive),
        "Different directives should not be equal",
      );
      assertFalse(
        twoParams1a.layer.equals(twoParams3.layer),
        "Different layers should not be equal",
      );
      assertFalse(
        twoParams1a.profile.equals(twoParams4.profile),
