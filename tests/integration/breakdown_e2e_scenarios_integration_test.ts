...
Deno.test("E2E Integration - Help command accessibility", async () => {
  ...
  assertStringIncludes(result.stdout.toLowerCase(), "usage", "Help should contain usage information");
});
...
Deno.test("E2E Integration - Large input processing", async () => {
  ...
  try {
    ...
    const result = await runBreakdownCommand([
      "summary", "project",
      "--to-file", outputFile
    ], largeInput, 45000); // Extended timeout for large input
...
Deno.test("E2E Integration - Concurrent command execution", async () => {
  ...
  try {
    ...
    assertEquals(successCount > 0, true, "At least one concurrent command should succeed");
...
Deno.test("E2E Integration - Command timeout behavior", async () => {
  ...
  const result = await runBreakdownCommand([
    "to", "project",
    "--from-file", "/dev/null"
  ], undefined, 100); // Very short timeout
...
Deno.test("E2E Integration - Signal handling", async () => {
  ...
  const cmd = new Deno.Command("deno", {
    args: ["run", "--allow-all", "./main.ts", "to", "project", "--from-file", "/dev/null"],
    stdout: "piped",
    stderr: "piped"
  });
