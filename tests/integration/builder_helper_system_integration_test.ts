...
Deno.test("Builder-Helper Integration - VariablesBuilder with Factory values", async () => {
  ...
  if (factoryResult.ok) {
    ...
    assertEquals(builder.getVariableCount() > 0, true, "Builder should create variables from factory values");
    ...
    if (buildResult.ok) {
      ...
      assertEquals(record["uv-integration"], "integration test value", "Should include custom variables");
      assertEquals(record.input_text, "Integration test input content", "Should include stdin content");
    }
    
    logger.debug("VariablesBuilder factory integration successful");
  } else {
    logger.debug("Factory creation failed, skipping builder integration", { error: factoryResult.error });
  }
});
...
Deno.test("Builder-Helper Integration - Complete workflow validation", async () => {
  ...
  if (factoryResult.ok) {
    ...
    if (paramsResult.ok) {
      ...
      if (buildResult.ok) {
        const record = builder.toRecord();
        assertEquals(record["uv-directive"], params.directive.getValue(), "Should preserve directive value");
...
Deno.test("Builder-Helper Integration - Memory management", async () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (buildResult.ok) {
      const record = builder.toRecord();
      assertEquals(Object.keys(record).length, 3, "Each iteration should have correct variable count");
    }
...
Deno.test("Builder-Helper Integration - Type safety verification", async () => {
  ...
  if (factoryResult.ok) {
    ...
    assertEquals(typeof bundle.typeFactory.createBothTypes, "function", "TypeFactory should be properly typed");
    assertEquals(typeof bundle.patternProvider.hasValidPatterns, "function", "PatternProvider should be properly typed");
    assertEquals(typeof bundle.config.getConfig, "function", "Config should be properly typed");
    assertEquals(typeof bundle.createPromptFactory, "function", "PromptFactory creator should be function");
...
Deno.test("Builder-Helper Integration - Interface compatibility", async () => {
  ...
  const factoryResult = await createTotalityFactory();
  assertEquals(typeof factoryResult.ok, "boolean", "Factory result should implement Result pattern");
  
  const validationResult = await validateConfigurationPatterns();
  assertEquals(typeof validationResult.valid, "boolean", "Validation should have specific interface");
  assertEquals(Array.isArray(validationResult.details), true, "Validation should provide details array");
  ...
  if (factoryResult.ok) {
    ...
    assertEquals(typeof bundle.createPromptFactory, "function", "createPromptFactory should be function");
  }
