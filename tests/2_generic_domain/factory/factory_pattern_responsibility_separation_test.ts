...
class FactoryResponsibilityAnalyzer {
  ...
  analyzeResponsibilitySeparation(): {
    ...
    const properFactoryActions = this.creationActions.filter(a => 
      a.action.includes("create") || a.action.includes("instantiate") || a.action.includes("configure")
    ).length;
    
    const separationScore = totalActions > 0 ? 
      Math.round((properFactoryActions / totalActions) * 100) : 100;
...
Deno.test("Factory Pattern: PromptVariablesFactory responsibility separation", async () => {
  logger.debug("Testing PromptVariablesFactory responsibility separation", "pattern:prompt-variables-factory");
  ...
  // Business logic should be handled by delegated components
  ...
  assertEquals(creationMethods.length > 0, true, 
  ...
  assertEquals(businessLogicMethods.length, 0,
    "Factory should not have business logic methods");

  const analysis = analyzer.analyzeResponsibilitySeparation();
  
  logger.debug("PromptVariablesFactory analysis completed", "pattern:analysis", {
  ...
  assertEquals(analysis.separationScore >= 70, true, 
    `Separation score ${analysis.separationScore} should be >= 70`);
});

Deno.test("Factory Pattern: PathResolver responsibility separation", async () => {
  logger.debug("Testing PathResolver responsibility separation", "pattern:path-resolver");

  const analyzer = new FactoryResponsibilityAnalyzer();
  
  // Test InputFilePathResolver
  analyzer.recordFactoryCreation("InputFilePathResolver", "create");
  const inputResolverResult = InputFilePathResolver.create(mockConfig, 
  ...
  if (inputResolverResult.ok) {
    ...
    const inputMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(inputResolver))
      .filter(name => name !== 'constructor' && typeof (inputResolver as any)[name] === 'function');
    
    assertEquals(inputMethods.includes('getPath'), true, 
      "InputResolver should have getPath method");
    assertEquals(inputMethods.filter(m => m.includes('read') || m.includes('write')).length, 0,
  ...
  const outputResolverResult = OutputFilePathResolver.create(mockConfig,
  ...
  if (outputResolverResult.ok) {
    ...
    const outputMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(outputResolver))
      .filter(name => name !== 'constructor' && typeof (outputResolver as any)[name] === 'function');
    
    assertEquals(outputMethods.includes('getPath'), true,
      "OutputResolver should have getPath method");
    assertEquals(outputMethods.filter(m => m.includes('save') || m.includes('create')).length, 0,
      "OutputResolver should not have file creation methods");
  }

  const analysis = analyzer.analyzeResponsibilitySeparation();
  
  logger.debug("PathResolver analysis completed", "pattern:path-analysis", {
    factoryResponsibilities: analysis.factoryResponsibilities,
  ...
  assertEquals(analysis.separationScore >= 80, true,
    `Path resolver separation score ${analysis.separationScore} should be >= 80`);
});

Deno.test("Factory Pattern: abstraction and encapsulation verification", async () => {
  logger.debug("Testing factory abstraction and encapsulation", "pattern:abstraction");
  ...
  // Factory should have minimal public properties
  assertEquals(publicProperties.length <= 3, true,
    `Factory should have minimal public properties, got ${publicProperties.length}: ${publicProperties.join(', ')}`);
  ...
  assertEquals(interfaceComplexity <= 15, true,
  ...
  const alternativeFactory = new PromptVariablesFactory(alternativeConfig);
  ...
  assertEquals(originalMethods.length, alternativeMethods.length,
    "Factories with different configs should have the same interface");

  const analysis = analyzer.analyzeResponsibilitySeparation();
  
  logger.debug("Abstraction verification completed", "pattern:abstraction-verified", {
...
Deno.test("Factory Pattern: delegation and orchestration verification", async () => {
  logger.debug("Testing factory delegation and orchestration", "pattern:delegation");
  ...
  for (const { name, params, expectedDelegations } of orchestrationTestCases) {
    logger.debug(`Testing orchestration: ${name}`, "pattern:orchestration-case", {
  ...
  const parallelResults = await Promise.all(
    parallelParams.map(params => factory.create(params))
  );
  const endTime = performance.now();
  
  // Verify parallel orchestration
  assertEquals(parallelResults.length, parallelParams.length,
  ...
  logger.debug("Delegation verification completed", "pattern:delegation-verified", {
  ...
  assertEquals(analysis.separationScore >= 85, true,
    `Delegation separation score ${analysis.separationScore} should be >= 85`);
});

Deno.test("Factory Pattern: cross-factory responsibility consistency", async () => {
  logger.debug("Testing cross-factory responsibility consistency", "pattern:consistency");
  ...
  const factoryTestData = [
    {
      ...
      factory: new PromptVariablesFactory(mockConfig),
      createMethod: async (f: any) => await f.create(createTestParams("to", "project")),
    },
    {
      factoryType: "InputFilePathResolver",
      factory: null, // Static factory
      createMethod: (f: any) => InputFilePathResolver.create(mockConfig, createTestParams("to", "project")),
    },
    {
      factoryType: "OutputFilePathResolver", 
      factory: null, // Static factory
      createMethod: (f: any) => OutputFilePathResolver.create(mockConfig, createTestParams("to", "project")),
    },
  ];
  ...
  for (const { factoryType, factory, createMethod } of factoryTestData) {
    logger.debug(`Analyzing factory: ${factoryType}`, "pattern:factory-analysis", {
      factoryType,
      hasInstance: !!factory,
    });
    ...
    try {
      const result = await createMethod(factory);
      ...
      logger.debug(`Factory ${factoryType} analysis failed`, "pattern:analysis-error", {
        error: error.message,
      });
    }
  }
  ...
  assertEquals(factoryPatterns.length, factoryTestData.length,
    "All factories should be analyzable");

  // All factories should have creation responsibility
  for (const pattern of factoryPatterns) {
    assertEquals(pattern.responsibilities.includes('creation'), true,
  ...
  for (const staticFactory of staticFactories) {
    assertEquals(staticFactory.methodCount <= 5, true,
  ...
  for (const instanceFactory of instanceFactories) {
    assertEquals(instanceFactory.methodCount <= 20, true,
      `Instance factory ${instanceFactory.factoryType} should have <= 20 methods`);
  }

  const analysis = analyzer.analyzeResponsibilitySeparation();
  
  logger.debug("Cross-factory consistency verified", "pattern:consistency-verified", {
  ...
  assertEquals(analysis.separationScore >= 75, true,
    `Cross-factory separation score ${analysis.separationScore} should be >= 75`);
});