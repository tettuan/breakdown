...
class SystemInitializationManager {
  ...
  async concurrentInitialize(): Promise<{ success: boolean; results: Map<string, SystemComponent> }> {
    ...
    for (let level = 0; level < levelGroups.length; level++) {
      ...
      const initPromises = group.map(componentName => 
...
Deno.test("Cross-System Init: Concurrent initialization safety", async () => {
  ...
  // 初期化成功の確認
  assertEquals(result.success, true, "All components should initialize successfully in concurrent mode");
  ...
  for (const [name, component] of result.results) {
    if (component.initialized) {
      ...
      for (const dep of component.dependencies) {
        ...
        assertEquals(depInitIndex < componentInitIndex, true,
...
Deno.test("Cross-System Init: Initialization failure recovery", async () => {
  ...
  try {
    ...
    const err = error as Error;
    assertStringIncludes(err.message, "Dependency", 
...
Deno.test("Cross-System Init: Resource cleanup verification", async () => {
  ...
  try {
    ...
    // 高いクリーンアップ成功率を期待
    assertEquals(successfulCleanups.length >= cleanupResults.length * 0.9, true,
...
Deno.test("Cross-System Init: Initialization state consistency", async () => {
  ...
  for (let cycle = 0; cycle < 3; cycle++) {
    ...
    // 状態の一貫性確認
    for (const [name, component] of result.results) {
      assertEquals(component.initialized, true, 
        `Component ${name} should be initialized in cycle ${cycle + 1}`);
      assertExists(component.initTime, 
