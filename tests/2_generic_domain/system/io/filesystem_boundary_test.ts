...
Deno.test("Filesystem Boundary: Concurrent access handling", async () => {
  ...
  try {
    ...
    // 高い成功率を期待（80%以上）
    assertEquals(
      writeSuccessCount >= Math.floor(writeResults.length * 0.8),
      true,
      `Write success rate should be at least 80%, got ${writeSuccessCount}/${writeResults.length}`,
    );
    assertEquals(
      readSuccessCount >= Math.floor(readResults.length * 0.8),
      true,
...
Deno.test("Filesystem Boundary: Special characters and encoding", async () => {
  ...
  try {
    ...
    const specialCharTests = [
      ...
      {
        name: "mixed",
        content: "Mixed: English + 日本語 + Français + 🎌",
        filename: "mixed_encoding.txt",
      },
