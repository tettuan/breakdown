 ...
 * 3. 適切な終了コード相当の処理
 */
Deno.test("E2E Error Handling: S3.1 - Invalid Arguments", async () => {
  ...
  const invalidArgumentCases = [
    {
      ...
      expectedErrorType: "InvalidInput",
      expectedErrorMessage: "does not match required pattern"
    },
    {
      name: "invalid_layer", 
      args: ["to", "invalid_layer", "input.md"],
      expectedErrorStage: "layer_validation",
      expectedErrorType: "InvalidInput",
      expectedErrorMessage: "does not match required pattern"
    },
    {
      name: "both_invalid",
      args: ["bad_directive", "bad_layer", "input.md"],
      expectedErrorStage: "both_validation",
      expectedErrorType: "InvalidInput",
      expectedErrorMessage: "does not match required pattern"
    },
    {
  ...
  for (const testCase of invalidArgumentCases) {
    ...
    assertEquals(hasExpectedError, true, 
      `Should detect error for ${testCase.name}`);
    
    if (hasExpectedError) {
      assertEquals(actualErrorType, testCase.expectedErrorType,
        `Error type should be ${testCase.expectedErrorType} for ${testCase.name}`);
      
      assertStringIncludes(actualErrorMessage, testCase.expectedErrorMessage,
    ...
    assertEquals(errorDetails.canProceed, false,
 ...
 */
Deno.test("E2E Error Handling: S3.2 - Filesystem Errors", async () => {
  ...
  for (const testCase of filesystemErrorCases) {
    ...
    assertEquals(fileAccessResult.success, false,
      `File access should fail for ${testCase.name}`);
    assertEquals(fileAccessResult.error.kind, testCase.expectedError,
 ...
 */
Deno.test("E2E Error Handling: S3.3 - Configuration Errors", async () => {
  ...
  for (const testCase of configErrorCases) {
    ...
    assertEquals(fallbackConfig.success, true,
      `Fallback config should be applied for ${testCase.name}`);
    assertEquals(fallbackConfig.strategy, testCase.expectedRecovery,
 ...
 */
function simulateFileAccess(filePath: string, simulatedError: any) {
  return {
    success: false,
 ...
 * エラーに基づいてリカバリー戦略を決定
 */
function determineRecoveryStrategy(error: any) {
 ...
 */
function simulateConfigLoad(configContent: any) {
  ...
  if (configContent.directiveTypes?.patterns?.includes("[invalid")) {
    return {
      success: false,
      error: {
 ...
 * フォールバック設定を適用
 */
function applyFallbackConfig(error: any) {
  const fallbackStrategies: Record<string, any> = {
    "ConfigLoadError": {
  ...
  const strategy = fallbackStrategies[error.kind];
  return {
    success: true,
    ...strategy
  };
