...
Deno.test("Architecture: Examples follow numbered naming convention", async () => {
  ...
  const testFiles = ["test_custom_vars.sh", "test_breakdown_config.ts", "test_direct_config.ts", 
                     "test_input.md", "test_params_access.ts", "test_yaml_load.ts", "debug_config.ts"];
  ...
  for await (const entry of walk(examplesDir, { maxDepth: 1 })) {
    if (entry.isFile) {
      ...
      if (entry.path.includes("/output/") || 
          entry.path.includes("/prompts/") ||
          entry.path.includes("/.agent/")) continue;
...
Deno.test("Architecture: Examples have distinct purposes", async () => {
  ...
  const allowedDuplicates = ["config"];
