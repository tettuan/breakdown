...
Deno.test("EnhancedParamsParser - CustomConfig vs No Config Consistency", async (t) => {
  await t.step("should handle 'find bugs' consistently without custom config", () => {
    ...
    // Without custom config, "find bugs" should be treated as three-word command
    assertEquals(result.type, "three");

    // deno-lint-ignore no-explicit-any
    const threeResult = result as any;
    assertEquals(threeResult.demonstrativeType, "find");
    assertEquals(threeResult.subCommand, "bugs");
    assertEquals(threeResult.layerType, undefined);
  });
  ...
  await t.step("should handle three-word commands consistently", () => {
    ...
    // Both should treat "find bugs layer" as three-word command
    const result1 = parserNoConfig.parse(["find", "bugs", "component"]);
    const result2 = parserWithConfig.parse(["find", "bugs", "component"]);

    assertEquals(result1.type, "three");
    assertEquals(result2.type, "three");

    // Both should have same structure
    // deno-lint-ignore no-explicit-any
    const three1 = result1 as any;
    // deno-lint-ignore no-explicit-any
    const three2 = result2 as any;

    assertEquals(three1.demonstrativeType, "find");
    assertEquals(three1.subCommand, "bugs");
    assertEquals(three1.layerType, "component");

    assertEquals(three2.demonstrativeType, "find");
    assertEquals(three2.subCommand, "bugs");
    assertEquals(three2.layerType, "component");
  });
});
...
Deno.test("EnhancedParamsParser - BreakdownParams Limitations Documentation", async (t) => {
  ...
  await t.step("should document equals format preprocessing", () => {
    ...
    // Test that equals format is preprocessed but still fails due to arg count
    const result = parser.parse(["find", "bugs", "--from=test.js"]);
    ...
    assertEquals(threeResult.options.from, "test.js");
  });
  ...
  await t.step("should handle unknown options correctly", () => {
    ...
    // deno-lint-ignore no-explicit-any
    const threeResult = result as any;
    assertEquals(threeResult.options["unknown-option"], "value");
  });
});
...
Deno.test("EnhancedParamsParser - Edge Cases and Backwards Compatibility", async (t) => {
  ...
  await t.step("should handle custom variables in three-word commands", () => {
    const parser = new EnhancedParamsParser();
    ...
    // deno-lint-ignore no-explicit-any
    const threeResult = result as any;
    assertEquals(threeResult.layerType, "layer");

    // Custom variables should be properly parsed
    const customVars = threeResult.options.customVariables as Record<string, string>;
