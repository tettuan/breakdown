...
import { InputFilePathResolver } from "$lib/factory/InputFilePathResolver.ts";
...
describe("InputFilePathResolver: fromFile not provided/absolute/relative", () => {
  it("returns empty string if fromFile is not provided", () => {
    const resolver = new InputFilePathResolver({}, { layerType: "project", options: {} });
  ...
  it("returns absolute path if fromFile is absolute", () => {
    ...
    const resolver = new InputFilePathResolver({}, { layerType: "project", options: { fromFile: absPath } });
  ...
  it("resolves path if fromFile has path hierarchy (relative)", () => {
    ...
    const resolver = new InputFilePathResolver({}, { layerType: "project", options: { fromFile: relPath } });
...
describe("InputFilePathResolver: layerType/fromLayerType handling", () => {
  it("resolves path if fromFile is filename only (uses layerType)", () => {
    const resolver = new InputFilePathResolver({}, { layerType: "issue", options: { fromFile: "input.md" } });
    const expected = resolve(Deno.cwd(), "issue", "input.md");
    assertEquals(resolver.getPath(), expected);
  });
  it("resolves path if fromFile is filename only (uses fromLayerType)", () => {
    const resolver = new InputFilePathResolver({}, { layerType: "task", options: { fromFile: "input.md", fromLayerType: "project" } });
    const expected = resolve(Deno.cwd(), "project", "input.md");
    assertEquals(resolver.getPath(), expected);
  });
});
...
describe("InputFilePathResolver: Windows/edge/ambiguous cases", () => {
  it("normalizes Windows-style path separators", () => {
    ...
    const resolver = new InputFilePathResolver({}, { layerType: "project", options: { fromFile: winPath } });
    assertEquals(resolver.getPath(), expected);
  });
  it("handles edge case with empty layerType and fromLayerType", () => {
    const resolver = new InputFilePathResolver({}, { layerType: "", options: { fromFile: "input.md" } });
  ...
  it("handles ambiguous case where fromFile is a directory name", async () => {
    ...
    const resolver = new InputFilePathResolver({}, { layerType: "issue", options: { fromFile: dirName } });
    // Should resolve to <cwd>/issue/ambiguous.md (not treat as directory)
    const expected = resolve(Deno.cwd(), "issue", dirName);
    assertEquals(resolver.getPath(), expected);
