...
Breakdown は「核心ドメイン／支援ドメイン／汎用ドメイン」の 3 層で責務を分離します。本セクションは概要のみ示します。各ドメインの構成・設計原則・利用パターンは、それぞれの README を参照してください。

### 核心ドメイン（Core Domain）

**プロンプト生成ドメイン**がBreakdownの存在価値です。以下の4つの境界コンテキストで構成されています：

1. **プロンプトパス決定ドメイン** - ユーザーの意図を正確なファイルパスに変換 → [prompt_template_path.ja.md](./domain_core/prompt_template_path.ja.md)
2. **プロンプト変数生成ドメイン** - 入力データを完成されたプロンプト変数に変換 → [prompt_variables.ja.md](./domain_core/prompt_variables.ja.md)
3. **パラメータ解析ドメイン** - CLI引数を型安全なドメインオブジェクトに変換 → [two_params_types.ja.md](./domain_core/two_params_types.ja.md)
4. **設定管理ドメイン** - 環境固有の設定を統一的に管理 → [option_types.ja.md](./domain_core/option_types.ja.md)

ドメイン間の境界と協働は [domain_boundaries_flow.ja.md](./domain_core/domain_boundaries_flow.ja.md) を参照してください。

### 支援ドメイン（Supporting Domain）

**環境構築ドメイン**は、核心ドメインが機能するための基盤を提供します。構成・設計原則・利用パターンの詳細は [supporting_domain/README.md](./supporting_domain/README.md) を参照してください。
...
**技術基盤ドメイン**は、システム全体の技術的な基盤を提供します。構成・設計原則・JSR パッケージ責務の詳細は [generic_domain/README.md](./generic_domain/README.md) を参照してください。
...
Breakdown は **ドメインの境界を明確にする設計意図** により、以下の 4 つの JSR パッケージを利用しています：

- **[@tettuan/breakdownconfig](https://jsr.io/@tettuan/breakdownconfig)** - 設定管理
- **[@tettuan/breakdownparams](https://jsr.io/@tettuan/breakdownparams)** - パラメータ解析
- **[@tettuan/breakdownprompt](https://jsr.io/@tettuan/breakdownprompt)** - プロンプト生成
- **[@tettuan/breakdownlogger](https://jsr.io/@tettuan/breakdownlogger)** - ログ出力（テスト専用）

各パッケージの責務・他ドメインとの境界・利用方針の詳細は [generic_domain/README.md](./generic_domain/README.md#jsr-パッケージ責務) を参照してください。
...
詳細フローは [processing-flow.ja.md](./generic_domain/system/overview/processing-flow.ja.md) を参照してください。
...
#### 2.3 ワークスペース管理
ワークスペースの構成・初期化手順は [supporting_domain/README.md](./supporting_domain/README.md) を参照してください。
...
3. **[支援ドメイン概要](./supporting_domain/README.md)** - ワークスペース構築と初期化
...
Breakdown 全体に通底する 4 つの設計原則です：

1. **ドメイン駆動設計の適用** - 核心ドメインの価値最大化と境界コンテキストによる責務分離
2. **型安全性の確保** - Smart Constructors とパターンベースバリデーション
3. **拡張性の確保** - プロファイル切り替え／JSR パッケージ／テンプレート駆動
4. **ユーザー体験の最適化** - 確実性の保証と明確なエラーメッセージ

各ドメイン固有の設計原則・利用パターン・拡張戦略は、対応する README を参照してください：
- 支援ドメイン: [supporting_domain/README.md](./supporting_domain/README.md)
- 汎用ドメイン: [generic_domain/README.md](./generic_domain/README.md)
- 核心ドメイン境界: [domain_core/domain_boundaries_flow.ja.md](./domain_core/domain_boundaries_flow.ja.md)
