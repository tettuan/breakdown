...
> **このファイルの役割**: Breakdown を実装・改修するエンジニアが、コードを書く前に読むべきドキュメントの推奨読書順を示します。
>
> **index.ja.md との違い**:
> - **CLAUDE.md（本ファイル）** = 実装者向けの「読む順」。基本理解 → ドメイン詳細 → テストの段階的順序を示す。
> - **[index.ja.md](./index.ja.md)** = 機能別ガイド。ユーザー・開発者向けの仕様書インデックスで、初学者・中級者・上級者ごとの「利用ガイド」と全文書の目次を提供する。
>
> 機能別の入口を探している場合は [index.ja.md](./index.ja.md) を参照してください。本ファイルはコード変更前の読書順に特化します。
