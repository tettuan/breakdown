...
Smart Constructor 基底クラス（`ValidatedValue<T>`）の実装パターンと全域性原則の詳細は [totality.ja.md](../generic_domain/system/overview/totality.ja.md) を参照してください。

### ドメインバリューオブジェクト

> **DirectiveType / LayerType / ConfigProfile / TwoParams の型定義詳細**は [two_params_types.ja.md](./two_params_types.ja.md) を参照してください。本ドキュメントでは、これらの型がドメイン境界を跨ぐ際の役割と、境界に固有の型のみを扱います。

```typescript
// 設定分離型（BreakdownParams検証により簡素化）
// DirectiveType / LayerType の定義は two_params_types.ja.md を参照
type BreakdownConfig = {
  readonly profileName: ConfigProfile;
  // BreakdownParams検証済みのため、directivePatterns/layerPatternsは不要
  readonly app_prompt: { base_dir: string; working_dir?: string };  // PATH用
  readonly app_schema: { base_dir: string };                        // PATH用

  // BreakdownParams検証済み値は追加検証不要
...
```typescript
// BreakdownParams TwoParamsResult から境界を跨いで TwoParams を生成するヘルパー
// （DirectiveType / LayerType / TwoParams 自体の型定義は two_params_types.ja.md を参照）
function fromTwoParamsResult(jsrResult: TwoParamsResult): TwoParams {
  return {
    directive: new DirectiveType(jsrResult.directiveType),
    layer: new LayerType(jsrResult.layerType)
  };
}
...
### 実装チェックリスト
- [DONE] BreakdownParams検証済み値の直接利用（詳細は [two_params_types.ja.md](./two_params_types.ja.md) を参照）
- [ ] Result型による例外の排除  
...
- `fromTwoParamsResult()` ヘルパーは境界変換固有の責務として残置、内部実装をコンストラクタ呼び出しに統一
- TwoParams 型定義の重複を削除（two_params_types.ja.md 側を正とする）
