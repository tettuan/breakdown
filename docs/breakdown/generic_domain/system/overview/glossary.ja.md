...
| **オプション型設計** | | | |
| シナリオ（オプション型文脈） | Scenario | - | `option_types.ja.md` で使用される CLI 利用者視点の概念。「情報表示」「管理操作」「プロンプト生成」の 3 つの利用シナリオを指す。内部の「ドメイン境界」（パラメータ解析・パス解決等）とは別の抽象レイヤーであり、用語の混同を回避するため「シナリオ」に統一されている。詳細は [option_types.ja.md](../../../domain_core/option_types.ja.md) を参照。 |
