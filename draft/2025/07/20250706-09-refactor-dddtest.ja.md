...
5. `deno test  <task>` で、作成したファイルをテストする （allowフラグつけて）
6. 3で作成したリストの全てが完了するまで、再び4に戻り、実装修正を行う

### 全ての変更が完了した後

7. `tests/` 配下の.3_core,4_cross_domain のテスト実装を進める。
8. `deno task ci --single-file` を通して、passするまでテストを完成させる
