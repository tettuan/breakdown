...
信じて良い前提: Breakdown本体の主な処理は正しいが、不要な実装がある
課題: factoryを使えていない
タスク: lib/breakdown.tsのhandleTwoParams処理を、factory内のクラスへ移譲する
...
1. `docs/breakdown/app_factory.ja.md` を読む
2. factory/ 内を調べ、cli/breakdown.tsの置き換えられるロジックを見つける
3. cli/breakdown.tsがfactoryを用いるように変更する

### 必読資料

- アーキテクチャ `docs/breakdown/architecture.ja.md` に照らして判断する。
- 用語集は `docs/breakdown/glossary.ja.md` にある。
- `docs/breakdown/app_factory.ja.md` を読む
- その他仕様書は docs/ 配下にある。
...
- cli/breakdown.ts がfactoryへ移譲している
