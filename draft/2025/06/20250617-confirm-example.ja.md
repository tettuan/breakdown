...
信じて良い前提: Breakdown本体はテストをpassして正常に動作しており、examples/ 配下のスクリプトも正常動作する
タスク: examples/ 配下のスクリプトに対し、スクリプトが存在する場所をCWDとしてから実行するよう、冒頭にCWD移動処理を追加する。スクリプト完了後とエラーによる以上終了時には、移動前のCWDに戻る処理も入れる。
ゴール: 全てのexamplesに処理が入れられている状態とする

**注意書き**
実行前に、 `examples/README.ja.md` を読むこと。
実行結果ログは tmp/examples_result/ 配下に**1ファイルずつ**保存すること。
実行結果ログファイルを分析した結果も、同じ階層に保存すること。
