...
信じて良い前提: Breakdown本体の主な機能は正しいが、テストに失敗している
タスク: カスタム設定を動作させ、テストをpassさせる
ゴール: `deno task ci`が動作する
...
- テスト:
  - $prefix の値を何か適当なものに決める
    - 仮に `$prefx=staging` とする
  - `$prefix-user.yml` に、BreakdownParams へ渡すカスタム設定を持たせ、ファイルを作成する
    - docs/breakdown/breakdownparams/* に資料あり
    - 例えば `find`,`system`をパラメータ許容する
  - breakdownをユースケース通りに実行する。 
    - 例えば `breakdown find system --config=staging`
    - `--config=$prefx`
- 実装 `cli/breakdown.ts` :
  - prefix取得
  - BreakdownConfigをprefix付きで呼び出す
    - $prefix-app.yml と $prefix-user.yml とを結合するのはBreakdownConfig内部で行われる。Breakdown本体では行わない。
  - BreakdownConfigの結果から、カスタム設定を取得
  - BreakdownParams へカスタム設定を渡す
  - BreakdownParamsのResultを検証する
