...
- タスク: タイムアウトの調査を行い、タイムアウト発生源を突き止める
- 作業リスト:
  - タイムアウトエラーが発生する場所を探す
  - コマンドの一部を変えて、違うパターンに変えても発生するか試す
  - 確実に内容が存在するファイルを使って渡す
  - STDINの渡し方の違いを調べる
- 解決すべき課題: 
  - "存在しないファイル待ち問題": STDINが渡されていないのに、STDIN待ちになる箇所を洗い出す
  - "タイムアウト時間設定問題": タイムアウトの時間設定を行なっている箇所を探す
- ゴール: 問題の発生箇所の候補リストが完成している。そのスクリプト実行コマンドや、結果、考えられる要因、を羅列する。
```
