...
なお、すでにpaneが存在するか確認し、存在する場合はさらに、node(claude) 起動をpane一覧で確認する。
...
**pane数**: main: 1, manager: 2, secretary: 1, worker: 9
idが小さい順に割り当てる。

`tmp/worker-management.md`へ割り当てを記録:
...
ワーカーがDONE状態になったら即座に`/clear`を指示してIDLE状態に復帰させる。

## 部下の役割とゴルーチンタイプ
- **永続ゴルーチン** (コンテキスト維持、pane id 小さい順3paneが対象):
...
3. ワーカーへプロジェクトルール（主目的、禁止事項）を通達 ( `instructions/project-worker.md` へ10行以内で記録する) 
