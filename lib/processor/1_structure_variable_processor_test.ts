...
Deno.test("Structure: Variable Processor should have single responsibility", () => {
  const processor = new TwoParamsVariableProcessor();
  
  // Primary responsibility: Variable processing
  assertExists(processor.process);
  assertEquals(typeof processor.process, "function");
  
  // Secondary responsibility: Custom variable extraction
  assertExists(processor.extractCustomVariables);
  assertEquals(typeof processor.extractCustomVariables, "function");
  ...
  nonResponsibilities.forEach(method => {
    assertEquals((processor as any)[method], undefined, 
      `Processor should not have ${method} method - violates single responsibility`);
  });
});

Deno.test("Structure: Variable Processor should have proper method organization", () => {
  const processor = new TwoParamsVariableProcessor();
  
  // Public interface methods
  const publicMethods = ["process", "extractCustomVariables"];
  publicMethods.forEach(method => {
    assertEquals(typeof (processor as any)[method], "function", 
...
Deno.test("Structure: Variable Processor should properly encapsulate dependencies", () => {
  const processor = new TwoParamsVariableProcessor();
  ...
  assertEquals((processor as any).stdinFactory, undefined,
    "Internal StdinVariableFactory should not be exposed");
  
  // Should not expose builder instances directly
  assertEquals((processor as any).builder, undefined,
...
Deno.test("Structure: Variable Processor should have cohesive method grouping", async () => {
  ...
  const customVars = processor.extractCustomVariables({
...
Deno.test("Structure: Variable Processor should have minimal coupling", () => {
  // Test that the processor can work with minimal external dependencies
  const processor = new TwoParamsVariableProcessor();
  
  // Should work with empty options
  const emptyResult = processor.extractCustomVariables({});
...
Deno.test("Structure: Variable Processor should have clear abstraction levels", async () => {
  ...
  const highLevel = await processor.process({
  ...
  const midLevel = processor.extractCustomVariables({
    "uv-test": "value",
    "regular": "ignored"
  });
...
Deno.test("Structure: Variable Processor should maintain state consistency", async () => {
  ...
  const call1 = processor.extractCustomVariables({ "uv-a": "1" });
  const call2 = processor.extractCustomVariables({ "uv-b": "2" });
  ...
  const process1 = await processor.process({
    options: { "uv-x": "first" }
  });
  
  const process2 = await processor.process({
    options: { "uv-y": "second" }
  });
...
Deno.test("Structure: Variable Processor should have appropriate error granularity", () => {
  const processor = new TwoParamsVariableProcessor();
  
  // Different types of errors should be distinguishable
  
  // Invalid option error
  const invalidResult = processor.extractCustomVariables({
    "uv-invalid": null
  });
...
Deno.test("Structure: Variable Processor should support composition", async () => {
  ...
  const step1 = processor.extractCustomVariables({
    "uv-step1": "value1"
  });
  ...
  if (step1.ok) {
    ...
    const step2 = await processor.process({
      options: {
        ...step1.data,
        "uv-step2": "value2"
      }
...
Deno.test("Structure: Static methods should complement instance methods", () => {
