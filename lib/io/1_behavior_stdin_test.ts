...
import {
  ...
  formatStdinError,
  hasStdinContent as _hasStdinContent,
  ...
  type StdinError as _StdinError,
  type StdinErrorType,
...
describe("Totality Verification: Error handling completeness", () => {
  ...
  it("should format all error types correctly", () => {
    ...
    for (const err of errors) {
      ...
      assert(
        hasRelevantTerm,
        `Formatted error "${formatted}" should contain relevant term for ${err.kind}`,
      );
    }
  });
  ...
  it("should verify no exceptions escape from safe functions", async () => {
    ...
    try {
      ...
      const _readResult = await readStdinSafe(validConfig.data);
      ...
      const _writeResult = writeStdoutSafe("test", writeConfig.data);
      // Should return Result, not throw
