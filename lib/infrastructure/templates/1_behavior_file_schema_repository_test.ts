...
import { 
  ...
  type FileSchemaRepositoryConfig 
} from "./file_schema_repository.ts";
import {
  Schema,
  SchemaPath,
  SchemaContent
...
Deno.test("FileSchemaRepository loadSchema - successful load with validation", async () => {
  const { repository, tempDir, cleanup } = await createTestSchemaRepository();
  
  try {
    // Create test schema file
    await createTestSchemaFile(tempDir, "to", "project", "test.json", sampleValidSchema);
...
Deno.test("FileSchemaRepository loadSchema - throws SchemaValidationError for invalid JSON", async () => {
  ...
  try {
    ...
    const schemaDir = join(tempDir, "schema", "to", "project");
    await ensureDir(schemaDir);
...
Deno.test("FileSchemaRepository loadSchemas - parallel loading with mixed results", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "schema1.json", sampleValidSchema);
    await createTestSchemaFile(tempDir, "summary", "task", "schema2.json", {
      "type": "object",
...
Deno.test("FileSchemaRepository getDependencies - returns empty array (simplified implementation)", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "refs.json", schemaWithReferences);
...
Deno.test("FileSchemaRepository listAvailable - returns manifest with metadata", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "user.json", schemaWithMeta);
    await createTestSchemaFile(tempDir, "summary", "task", "task.json", sampleValidSchema);
...
Deno.test("FileSchemaRepository listAvailable - filters by directive", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "convert.json", sampleValidSchema);
    await createTestSchemaFile(tempDir, "summary", "project", "summarize.json", sampleValidSchema);
...
Deno.test("FileSchemaRepository cache behavior - respects TTL and invalidation", async () => {
  ...
  try {
    // Create schema file
    await createTestSchemaFile(tempDir, "to", "project", "ttl.json", sampleValidSchema);
    ...
    const filePath = join(tempDir, "schema", "to", "project", "ttl.json");
    ...
    const content2 = schema2.getContent().getContent() as any;
...
Deno.test("FileSchemaRepository refresh - clears cache and manifest", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "cached.json", sampleValidSchema);
    ...
    await createTestSchemaFile(tempDir, "summary", "task", "new.json", sampleValidSchema);
...
Deno.test("FileSchemaRepository cache disabled - always loads from file", async () => {
  ...
  try {
    // Create schema file
    const filePath = await createTestSchemaFile(tempDir, "to", "project", "nocache.json", sampleValidSchema);
...
Deno.test("FileSchemaRepository validateOnLoad disabled - skips validation", async () => {
  ...
  try {
    ...
    await createTestSchemaFile(tempDir, "to", "project", "loose.json", looseSchema);
