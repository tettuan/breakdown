...
async function createTestConfigFiles(baseDir: string): Promise<void> {
  ...
  // Create test YAML files directly
...
Deno.test("1_behavior - Pattern caching behavior", () => {
...
Deno.test("1_behavior - Cache clearing and reloading", () => {
  const provider = new DefaultPatternProvider();
...
Deno.test("1_behavior - Debug information provides accurate state", () => {
  const provider = new DefaultPatternProvider();
...
Deno.test("1_behavior - Provider state consistency across operations", () => {
  const provider = new DefaultPatternProvider();
