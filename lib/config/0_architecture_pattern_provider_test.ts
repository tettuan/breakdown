...
Deno.test("Architecture: Import statement analysis and dependency graph", async () => {
  ...
  const sourceCode = await Deno.readTextFile("./lib/config/pattern_provider.ts");
...
Deno.test("Architecture: No circular dependencies", async () => {
  ...
  assertEquals(hasCircularDep, false, "ConfigPatternProvider should not have circular dependencies");
});
 ...
 */
Deno.test("Architecture: Consistent error handling strategy", () => {
...
Deno.test("Architecture: Private method encapsulation", () => {
  ...
  for (const method of privateMethods) {
    ...
    if (descriptor) {
      ...
      assertExists(descriptor, `Private method ${method} exists but should not be exposed in public API`);
    }
  }
});
...
Deno.test("Architecture: Single Responsibility Principle", () => {
  ...
  const publicMethods = Object.getOwnPropertyNames(ConfigPatternProvider.prototype)
    ...
    .filter(name => typeof ConfigPatternProvider.prototype[name as keyof typeof ConfigPatternProvider.prototype] === "function")
    .filter(name => !["getConfigData", "getConfigDataSync", "extractDirectivePatternString", "extractLayerTypePatternString"].includes(name));
