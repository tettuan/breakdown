...
Deno.test("Architecture: Factory Integration - Dependency Injection Ready", async () => {
  ...
  const mockConfigResult = await createMockUnifiedConfig();
  assert(mockConfigResult.ok, "Mock config creation should succeed");
  if (mockConfigResult.ok) {
    const builder = new UnifiedFactoryBuilder(mockConfigResult.data);
...
async function createMockUnifiedConfig(): Promise<Result<UnifiedConfigInterface, Error>> {
  ...
  const mockPatternProvider = {
    getAllPatterns: () => ({
      ok: true,
      data: { directiveTypes: ["to"], layerTypes: ["task"], customPatterns: null },
    }),
  };
  ...
  const mockPathOptions = {
    ...
    baseDir: "/test",
    resolve: () => ({ ok: true, data: "/test/resolved" }),
  };
  ...
  return {
    ok: true,
    data: mockInterface as unknown as UnifiedConfigInterface,
  };
}
