 ...
 */
Deno.test("Unit: ConfigPatternProvider retrieves patterns from direct configuration", () => {
 ...
 */
Deno.test("Unit: ConfigPatternProvider retrieves patterns from nested twoParamsRules", () => {
  const mockConfig = {
    getConfig: () => ({
      twoParamsRules: {
        directive: {
 ...
 */
Deno.test("Unit: ConfigPatternProvider retrieves patterns from validation structure", () => {
  const mockConfig = {
    getConfig: () => ({
      validation: {
        directive: {
 ...
 */
Deno.test("Unit: ConfigPatternProvider falls back to default patterns", () => {
  const mockConfig = {
    getConfig: () => ({}), // 空の設定
    loadConfig: async () => {},
  } as unknown as BreakdownConfig;
 ...
 */
Deno.test("Unit: ConfigPatternProvider caches patterns", () => {
 ...
 */
Deno.test("Unit: ConfigPatternProvider clearCache clears pattern cache", () => {
  const mockConfig = {
    getConfig: () => ({
  ...
  assertEquals(debugInfo.cacheStatus.directive, "cached", "Directive should be cached after debug call");
  ...
  assertEquals(debugInfo.cacheStatus.directive, "cached", "Directive gets cached again when debug is called");
  assertEquals(debugInfo.cacheStatus.layer, "cached", "Layer gets cached again when debug is called");
});
 ...
 */
Deno.test("Unit: ConfigPatternProvider hasValidPatterns checks both patterns", () => {
  // 両方のパターンがある場合
  const validConfig = {
    getConfig: () => ({
  ...
  assertEquals(validProvider.hasValidPatterns(), true, "Should return true when both patterns exist");
  ...
  assertEquals(directiveOnlyProvider.hasValidPatterns(), true, "Should use defaults when pattern missing");
});
 ...
 */
Deno.test("Unit: ConfigPatternProvider handles config errors gracefully", () => {
  const mockConfig = {
    getConfig: () => {
...
Deno.test("Unit: ConfigPatternProvider.create factory method", async () => {
  ...
  await assertRejects(
    async () => {
      // 無効な設定名でエラーを発生させる
      await ConfigPatternProvider.create("__invalid__config__", "/nonexistent/path");
    },
    Error,
 ...
 */
Deno.test("Unit: ConfigPatternProvider debug output reflects current state", () => {
  const mockConfig = {
    getConfig: () => ({
  ...
  provider.clearCache();
  debugInfo = provider.debug();
  assertEquals(debugInfo.cacheStatus.directive, "cached", "Directive gets re-cached on debug after clear");
});
 ...
 */
Deno.test("Unit: ConfigPatternProvider handles invalid pattern strings", () => {
  const mockConfig = {
    getConfig: () => ({
 ...
 */
Deno.test("Unit: ConfigPatternProvider pattern extraction priority", () => {
  // 全ての設定場所にパターンがある場合
  const mockConfig = {
    getConfig: () => ({
      // 優先度1: 直接設定
