...
export class TwoParamsApplicationService {
  ...
  async process(
    ...
    if (!validationResult.ok) {
      return error({
        ...
        message: `TwoParams validation failed: ${this.formatValidationError(validationResult.error)}`,
      });
    }

    // Step 3: Process input data (STDIN, options)
    const inputProcessingResult = await this.processInput(input);
  ...
  private async processInput(
    input: TwoParamsInput,
  ): Promise<Result<void, TwoParamsApplicationServiceError>> {
    try {
      ...
      return error({
        kind: "InputProcessingError",
        message: `Failed to process input: ${cause instanceof Error ? cause.message : String(cause)}`,
        cause,
      });
    }
  }
  ...
  private async processVariables(
    ...
    stdinContent: string,
  ): Promise<Result<ProcessedVariables, TwoParamsApplicationServiceError>> {
    try {
      ...
      const processedVariables: ProcessedVariables = {
        ...
        user: {
          ...
          // Filter options that start with "uv-" or similar
        },
      };
...
`;
      ...
      return error({
        ...
        message: `Failed to generate prompt: ${cause instanceof Error ? cause.message : String(cause)}`,
        cause,
      });
    }
  }
