    ...
    switch (this.kind) {
      case "template-processing-failed":
        suggestions.push({
          ...
          description: "Check template file permissions",
        });
        suggestions.push({
          action: "check-encoding",
          description: "Ensure template file is valid UTF-8",
        });
        suggestions.push({
          action: "verify-path",
          description: "Verify template exists at the specified path",
        });
        break;
      case "variable-substitution-failed":
        suggestions.push({
          action: "define-variable",
          description: `Define variable '${this.context?.variableName}' in your input`,
        });
        suggestions.push({
          action: "check-typos",
        ...
        suggestions.push({
          action: "debug-variables",
          description: "See available variables",
          command: "--debug",
        });
        break;
      case "template-syntax-error":
        ...
        suggestions.push({
          action: "check-brackets",
          description: "Check for unclosed variable brackets: {{variable}}",
        });
        suggestions.push({
          action: "check-nesting",
          description: "Check for invalid nesting of conditionals",
        });
        suggestions.push({
          action: "check-escaping",
          description: "Check proper escaping of special characters",
        });
        if (this.context?.lineNumber) {
          suggestions.push({
            action: "check-line",
            description: `Error at line ${this.context.lineNumber}`,
          });
        }
        break;
      case "output-generation-failed":
        suggestions.push({
          action: "check-output-permissions",
          description: "Check output directory permissions",
        });
        suggestions.push({
          action: "check-writable",
          description: "Ensure output path is writable",
        });
        suggestions.push({
          action: "try-different-location",
          description: "Try a different output location",
        });
        break;
      case "schema-embedding-failed":
        suggestions.push({ action: "verify-schema", description: "Verify schema file exists" });
        suggestions.push({
          action: "check-schema-format",
          description: "Check schema file format",
        });
        suggestions.push({
          action: "check-schema-path",
          description: "Ensure schema path is correct",
        });
        break;
      case "circular-reference-detected":
        ...
        suggestions.push({
          action: "restructure",
          description: "Restructure your variables to avoid circular references",
        });
        break;
      case "max-depth-exceeded":
        suggestions.push({
          action: "reduce-nesting",
          description: "Reduce template nesting depth",
        });
        suggestions.push({
          action: "increase-limit",
          description: "Increase max depth limit in configuration",
        });
        suggestions.push({
          action: "simplify-structure",
          description: "Simplify template structure",
        });
        break;
    }
