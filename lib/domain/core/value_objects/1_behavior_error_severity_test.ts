...
import {
  ErrorMetadata,
  ErrorSeverity,
  ErrorSeverityError as _ErrorSeverityError,
  formatErrorSeverityError,
  ImpactScope,
  isInvalidImpactError,
  isInvalidLevelError,
  isInvalidMetadataError as _isInvalidMetadataError,
...
Deno.test("Architecture - ErrorMetadata interface supports extensible metadata", () => {
  ...
  const _emptyMetadata: ErrorMetadata = {};
  assertExists(_emptyMetadata);

  // Context should support any structure
  const _complexMetadata: ErrorMetadata = {
    context: {
      nested: {
        deep: {
          value: "test",
...
Deno.test("Totality - ErrorSeverity.fromString returns Result type with comprehensive validation", () => {
  ...
  const nullResult = ErrorSeverity.fromString(null as unknown as string);
...
Deno.test("Totality - formatErrorSeverityError provides comprehensive error messages", () => {
  ...
  const nullResult = ErrorSeverity.create(null as unknown as SeverityLevel, ImpactScope.MODULE);
  assert(!nullResult.ok);
  if (!nullResult.ok) {
...
Deno.test("Totality - ErrorSeverity instances are completely immutable", () => {
  ...
  if (result.ok) {
    ...
    const _metadata = severity.getMetadata();
