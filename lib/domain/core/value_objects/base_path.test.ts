...
import {
  ...
  type PathValidationError as _PathValidationError,
} from "./base_path.ts";
...
Deno.test("0_architecture: Template Method pattern in createPath", () => {
  ...
  const stages = [
    { input: null as unknown as string, expectedError: "EMPTY_PATH" },
...
Deno.test("1_behavior: basic path validation - empty paths", () => {
  const testCases = [
    ...
    { input: 123 as unknown as string, description: "non-string input" },
  ];
...
Deno.test("2_structure: immutability of created instances", () => {
  ...
  if (result.ok) {
    ...
    try {
      (path as unknown as { _value: string })._value = "modified";
