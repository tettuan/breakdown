...
Deno.test("2_structure: instances are completely immutable", () => {
  ...
  if (result.ok) {
    ...
    try {
      ...
      (workspace as unknown as { value: string }).value = "modified";
      (workspace as unknown as { newProp: string }).newProp = "added";
    } catch (e) {
    ...
    assertEquals((workspace as unknown as { newProp?: string }).newProp, undefined);
  }
});
...
Deno.test("2_structure: collection returns immutable views", () => {
  ...
  if (result.ok) {
    ...
    try {
      (names as unknown as string[]).push("new-workspace");
      (workspaces as unknown as WorkspaceName[]).push(null as unknown as WorkspaceName);
    } catch (e) {
