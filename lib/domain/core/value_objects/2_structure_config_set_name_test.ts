...
Deno.test("2_structure: instances are completely immutable", () => {
  ...
  if (result.ok) {
    ...
    try {
      ...
      (config as any).value = "modified";
      (config as any).newProp = "added";
    } catch (e) {
    ...
    assertEquals((config as any).newProp, undefined);
  }
});
...
Deno.test("2_structure: collection returns immutable views", () => {
  ...
  if (result.ok) {
    ...
    try {
      (names as any).push("new-config");
      (configs as any).push(null);
    } catch (e) {
