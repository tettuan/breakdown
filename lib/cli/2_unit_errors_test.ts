...
Deno.test("Unit: CliError throwing and catching behavior", () => {
  ...
  for (const code of errorCodes) {
    ...
    try {
      ...
      if (error instanceof CliError) {
        assertEquals(error.code, code, `Should have code property when caught as Error for ${code}`);
      }
    }
  }
});
...
Deno.test("Unit: CliError with empty and special messages", () => {
  ...
  for (const message of specialMessages) {
    ...
    assertEquals(error.code, CliErrorCode.INVALID_OPTION, "Should preserve code with special message");
    assertEquals(
      error.message,
...
Deno.test("Unit: CliError comparison and equality", () => {
  ...
  assertEquals(error1.message === error4.message, false, "Should have different messages with different input");
...
Deno.test("Unit: CliError with complex real-world scenarios", () => {
  ...
  assertStringIncludes(validationError.message, "Invalid timeout", "Should handle validation errors");
...
Deno.test("Unit: CliError edge cases and boundary conditions", () => {
  ...
  assertStringIncludes(formatError.message, "[INVALID_INPUT_TYPE]", "Should include code with formatting");
...
Deno.test("Unit: CliError toString and serialization behavior", () => {
  ...
  assertEquals(customSerialized.code, CliErrorCode.DUPLICATE_OPTION, "Should preserve code in custom format");
});