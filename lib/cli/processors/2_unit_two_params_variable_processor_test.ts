...
import { TwoParamsVariableProcessor, type VariableProcessorError } from "./two_params_variable_processor.ts";
...
Deno.test("TwoParamsVariableProcessor Units", async (t) => {
  ...
  await t.step("processVariables - empty value detection", () => {
    ...
    assert(!result.ok);
    if (!result.ok) {
      const emptyErrors = result.error.filter((e: VariableProcessorError) => e.kind === "EmptyVariableValue");
  ...
  await t.step("processVariables - option field mapping", () => {
    ...
    const result1 = processor.processVariables(options1, "");
    ...
    const result2 = processor.processVariables(options2, "");
    ...
    const result3 = processor.processVariables(options3, "");
  ...
  await t.step("processVariables - stdin content handling", () => {
    ...
    const result1 = processor.processVariables({}, "hello world");
    assert(result1.ok);
    if (result1.ok) {
    ...
    const result2 = processor.processVariables({}, "");
    assert(result2.ok);
    if (result2.ok) {
    ...
    const result3 = processor.processVariables({}, null as unknown as string);
    assert(result3.ok);
    if (result3.ok) {
  ...
  await t.step("processVariables - invalid options handling", () => {
    ...
    const stringResult = processor.processVariables("string" as unknown as Record<string, unknown>, "");
    assert(!stringResult.ok);
    
    const numberResult = processor.processVariables(42 as unknown as Record<string, unknown>, "");
  ...
  await t.step("processVariables - allVariables union behavior", () => {
    ...
    if (result.ok) {
      ...
      const expectedCount = Object.keys(customVariables).length + Object.keys(standardVariables).length;
