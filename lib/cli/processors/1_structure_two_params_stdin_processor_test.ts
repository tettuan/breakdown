...
const logger = new BreakdownLogger("structure-stdin-processor");

describe("TwoParamsStdinProcessor Structure - Class Design Principles", () => {
  it("should adhere to single responsibility principle", () => {
    ...
    const publicMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(processor))
      .filter(name => name !== "constructor" && !name.startsWith("_"));
    
    assertEquals(publicMethods.length, 3);
  ...
  it("should maintain clear separation between stdin detection and processing", async () => {
    logger.debug("Testing separation of concerns in stdin handling");
  ...
  it("should properly encapsulate internal state", async () => {
    ...
    assertEquals(typeof (processor as unknown as { shouldReadStdin?: () => boolean }).shouldReadStdin, "function");
  ...
  it("should maintain consistent interface patterns", async () => {
    logger.debug("Testing interface consistency patterns");
    
    const processor = new TwoParamsStdinProcessor();
    
    // Both methods should follow same Result<T, E> pattern
    const defaultResult = await processor.processWithDefaultTimeout({});
    const customResult = await processor.process(
  ...
  it("should structure resource management properly", async () => {
    logger.debug("Testing resource management structure");
...
describe("TwoParamsStdinProcessor Structure - Data Flow Patterns", () => {
  ...
  it("should maintain data integrity through processing pipeline", async () => {
    logger.debug("Testing data integrity through processing pipeline");
    ...
    for (const flow of optionFlows) {
      const result = await processor.process(config, flow.options);
...
describe("TwoParamsStdinProcessor Structure - Configuration Handling", () => {
  it("should handle configuration abstraction properly", async () => {
    logger.debug("Testing configuration abstraction handling");
    ...
    const result1 = await processor.process(minimalConfig, {});
    const result2 = await processor.process(extendedConfig, {});
  ...
  it("should structure timeout handling consistently", async () => {
    logger.debug("Testing timeout handling structure");
    
    const processor = new TwoParamsStdinProcessor();
    
    // Test timeout structure with convenience method
    const defaultTimeoutResult = await processor.processWithDefaultTimeout({
      skipStdin: true
    });
    
    // Test timeout structure with explicit config
    const explicitTimeoutResult = await processor.process(
  ...
  it("should maintain configuration validation boundaries", async () => {
    logger.debug("Testing configuration validation boundaries");
...
describe("TwoParamsStdinProcessor Structure - Method Organization", () => {
  it("should organize methods by abstraction level", async () => {
    logger.debug("Testing method organization by abstraction level");
    
    const processor = new TwoParamsStdinProcessor();
    
    // High-level method: processWithDefaultTimeout
    const highLevelResult = await processor.processWithDefaultTimeout({});
    assertExists(highLevelResult);
    
    // Mid-level method: process
    const midLevelResult = await processor.process(
      { stdin: { timeout_ms: 1000 } },
      {}
    );
  ...
  it("should maintain method signature consistency", async () => {
    ...
    const defaultResult = processor.processWithDefaultTimeout({});
  ...
  it("should structure convenience methods properly", async () => {
    logger.debug("Testing convenience method structure");
    ...
    const convenienceResult = await processor.processWithDefaultTimeout(options);
    const explicitResult = await processor.process(defaultConfig, options);
