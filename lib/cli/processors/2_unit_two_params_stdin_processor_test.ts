...
const logger = new BreakdownLogger("unit-stdin-processor");
...
describe("TwoParamsStdinProcessor Unit Tests - STDIN Detection Logic", () => {
  ...
  it("should correctly detect stdin flags", async () => {
    logger.debug("Testing stdin flag detection logic");
    ...
    const stdinFlagTests = [
      ...
      { options: { from: "-", fromFile: "file.txt" }, shouldReadStdin: true, desc: "from dash with file" },
      { options: { from: "file.txt", fromFile: "-" }, shouldReadStdin: true, desc: "fromFile dash with file" },
      ...
      { options: { from: "file1.txt", fromFile: "file2.txt" }, shouldReadStdin: false, desc: "both files" },
  ...
  it("should handle skipStdin flag correctly", async () => {
    logger.debug("Testing skipStdin flag handling");
  ...
  it("should handle edge case option values", async () => {
    logger.debug("Testing edge case option values");
    ...
    const edgeCaseTests = [
      ...
      { options: { from: undefined as unknown as string }, shouldReadStdin: false, desc: "undefined" },
...
describe("TwoParamsStdinProcessor Unit Tests - Timeout Configuration", () => {
  ...
  it("should handle various timeout configurations", async () => {
    logger.debug("Testing timeout configuration handling");
  ...
  it("should handle missing or invalid timeout configurations", async () => {
    logger.debug("Testing missing/invalid timeout configurations");
    ...
    const invalidTimeoutTests = [
      ...
      { config: { stdin: { timeout_ms: undefined as unknown as number } }, desc: "undefined timeout" },
  ...
  it("should use correct default timeout in convenience method", async () => {
    logger.debug("Testing default timeout in convenience method");
...
describe("TwoParamsStdinProcessor Unit Tests - Error Handling", () => {
  ...
  it("should handle all error scenarios gracefully", async () => {
    ...
    for (const scenario of errorScenarios) {
      const result = await processor.process(scenario.config, scenario.options);
  ...
  it("should preserve error cause information", async () => {
    logger.debug("Testing error cause preservation");
...
describe("TwoParamsStdinProcessor Unit Tests - Resource Management", () => {
  ...
  it("should clean up resources properly on success", async () => {
    logger.debug("Testing resource cleanup on success");
  ...
  it("should clean up resources properly on timeout", async () => {
    logger.debug("Testing resource cleanup on timeout");
  ...
  it("should handle abort controller lifecycle correctly", async () => {
    logger.debug("Testing abort controller lifecycle");
...
describe("TwoParamsStdinProcessor Unit Tests - State Management", () => {
  ...
  it("should maintain stateless operation", async () => {
    logger.debug("Testing stateless operation");
  ...
  it("should handle concurrent operations correctly", async () => {
    logger.debug("Testing concurrent operations");
    ...
    const operations = [
      ...
      processor.process(config, { from: "file2.txt" }),
      processor.processWithDefaultTimeout({ from: "file3.txt" }),
  ...
  it("should not share state between instances", async () => {
    logger.debug("Testing instance isolation");
    ...
    const result1 = await processor1.process(config, { from: "file.txt" });
    const result2 = await processor2.process(config, { from: "file.txt" });
...
describe("TwoParamsStdinProcessor Unit Tests - Integration Scenarios", () => {
  ...
  it("should handle realistic CLI usage patterns", async () => {
    logger.debug("Testing realistic CLI usage patterns");
  ...
  it("should work with various config sources", async () => {
    logger.debug("Testing various config sources");
  ...
  it("should demonstrate complete functionality", async () => {
    logger.debug("Testing complete functionality demonstration");
    ...
    // 1. No stdin reading
    const noStdinResult = await processor.process(config, { from: "file.txt" });
    ...
    const convenienceResult = await processor.processWithDefaultTimeout({ from: "file.txt" });
    ...
    // 4. Error handling
    const errorResult = await processor.process(null as unknown as BreakdownConfigCompatible, { from: "-" });
