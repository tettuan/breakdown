...
Deno.test({
  ...
  async fn(t) {
    await t.step("validates parameter count correctly", async () => {
      ...
      // Three parameters
      const threeParamsResult = await handleTwoParams(["to", "project", "extra"], {}, {
        skipStdin: true,
      });
    ...
    await t.step("validates demonstrative type correctly", async () => {
      ...
      const invalidResult = await handleTwoParams(["invalid_directive", "project"], {}, {
        skipStdin: true,
      });
...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step("handles special characters in parameters", async () => {
      const specialCharsResult = await handleTwoParams(["to@#$", "project!"], {}, {
        skipStdin: true,
      });
