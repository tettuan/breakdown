...
const logger = new BreakdownLogger("unit-handler");
...
describe("TwoParamsHandler Unit Tests - Parameter Validation", () => {
  it("should validate parameter count correctly", async () => {
    ...
    const parameterCountTests = [
      ...
      { params: ["one", "two", "three"], expectedValid: true, description: "three parameters (extra ignored)" },
    ];
    ...
    for (const test of parameterCountTests) {
      ...
      if (test.expectedValid) {
        ...
        if (!result.ok) {
          assert(result.error.kind !== "InvalidParameterCount", `Should not fail on parameter count for: ${test.description}`);
        }
      } else {
  ...
  it("should validate demonstrative types correctly", async () => {
    ...
    const demonstrativeTypeTests = [
      { type: "specification", valid: true },
      { type: "architecture", valid: true },
      { type: "example", valid: true },
      { type: "document", valid: true },
    ...
    for (const test of demonstrativeTypeTests) {
      const result = await handleTwoParams([test.type, "foundation"], {}, {});
      
      if (test.valid) {
        // May fail for other reasons, but not demonstrative type
        if (!result.ok) {
          assert(result.error.kind !== "InvalidDemonstrativeType", `Should not fail on demonstrative type for: ${test.type}`);
        }
      } else {
  ...
  it("should validate layer types correctly", async () => {
    ...
    const layerTypeTests = [
      { type: "foundation", valid: true },
      { type: "core", valid: true },
      { type: "integration", valid: true },
      { type: "scenarios", valid: true },
      { type: "invalid", valid: false },
    ...
    for (const test of layerTypeTests) {
      const result = await handleTwoParams(["specification", test.type], {}, {});
      
      if (test.valid) {
        // May fail for other reasons, but not layer type
        if (!result.ok) {
          assert(result.error.kind !== "InvalidLayerType", `Should not fail on layer type for: ${test.type}`);
        }
      } else {
...
describe("TwoParamsHandler Unit Tests - Input Processing", () => {
  it("should handle stdin input options correctly", async () => {
    logger.debug("Testing stdin input option handling");
    ...
    const stdinOptionTests = [
      ...
      { options: { from: "", fromFile: "" }, expectStdinRead: false, description: "empty input options" },
    ];
  ...
  it("should handle input text processing correctly", async () => {
    logger.debug("Testing input text processing");
...
describe("TwoParamsHandler Unit Tests - Configuration Handling", () => {
  it("should handle various configuration types", async () => {
    logger.debug("Testing configuration type handling");
    ...
    for (const test of configurationTests) {
      ...
      if (!result.ok && (result.error.kind === "FactoryValidationError" || result.error.kind === "StdinReadError")) {
...
describe("TwoParamsHandler Unit Tests - Variables and Prompt Generation", () => {
  it("should handle variables builder integration", async () => {
    logger.debug("Testing variables builder integration");
  ...
  it("should handle prompt generation correctly", async () => {
    logger.debug("Testing prompt generation");
...
describe("TwoParamsHandler Unit Tests - Error Handling", () => {
  it("should handle all error scenarios gracefully", async () => {
    ...
    for (const scenario of errorScenarios) {
      const result = await handleTwoParams(scenario.params, scenario.config as any, scenario.options);
  ...
  it("should provide detailed error information", async () => {
    logger.debug("Testing detailed error information");
...
describe("TwoParamsHandler Unit Tests - End-to-End Processing", () => {
  it("should handle complete processing workflow", async () => {
    logger.debug("Testing complete processing workflow");
  ...
  it("should handle concurrent processing correctly", async () => {
    ...
    const concurrentPromises = [
      ...
      handleTwoParams(["example", "integration"], config, {}),
    ];
  ...
  it("should demonstrate comprehensive functionality", async () => {
    logger.debug("Testing comprehensive functionality demonstration");
