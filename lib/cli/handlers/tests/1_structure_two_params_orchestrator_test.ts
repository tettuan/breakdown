...
import { TwoParamsOrchestrator, handleTwoParamsWithOrchestrator } from "../two_params_orchestrator.ts";
...
Deno.test({
  ...
  await t.step("maintains functional interface compatibility", async () => {
    ...
    const result = await handleTwoParamsWithOrchestrator(["demo", "layer"], {}, { skipStdin: true });
    assert(typeof result === "object");
    assert("ok" in result);
  });
  ...
  await t.step("provides equivalent functionality structure", async () => {
    ...
    const directResult = await orchestrator.orchestrate(["demo", "layer"], {}, { skipStdin: true });
    const functionResult = await handleTwoParamsWithOrchestrator(["demo", "layer"], {}, { skipStdin: true });
  ...
  await t.step("enforces consistent error mapping structure", async () => {
    ...
    for (const test of errorTests) {
      const directResult = await (new TwoParamsOrchestrator()).orchestrate(test.params, {}, { skipStdin: true });
      const functionResult = await handleTwoParamsWithOrchestrator(test.params, {}, { skipStdin: true });
