...
Deno.test({
  ...
  await t.step("maintains type safety boundaries", async () => {
    ...
    const nullResult = await orchestrator.orchestrate(null as any, {}, { skipStdin: true });
    assertEquals(nullResult.ok, false);
    
    const undefinedResult = await orchestrator.orchestrate(undefined as any, {}, { skipStdin: true });
