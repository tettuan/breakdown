...
import { showHelp, showVersion, showUsage, HELP_TEXT, APP_NAME, VERSION, type HelpTextConfig } from "./help.ts";
...
Deno.test("Help Module Structure", async (t) => {
  ...
  await t.step("maintains single responsibility per function", async () => {
    ...
    assertEquals(showVersionBody.includes("VERSION") || showVersionBody.includes("${VERSION}"), true);
  ...
  await t.step("separates data from presentation logic", async () => {
    ...
    assertEquals(helpContent.includes("const DEFAULT_HELP_CONFIG"), true, "Should define help config data");
    assertEquals(helpContent.includes("function generateHelpText"), true, "Should have text generation function");
    assertEquals(helpContent.includes("export const HELP_TEXT"), true, "Should export generated text");
  ...
  await t.step("maintains proper encapsulation", async () => {
    ...
    assertEquals(helpContent.includes("export function generateHelpText"), false, "generateHelpText should be private");
    
    // Internal data should not be exported
    assertEquals(helpContent.includes("export const DEFAULT_HELP_CONFIG"), false, "DEFAULT_HELP_CONFIG should be private");
    
    // Only public API should be exported
    const exports = helpContent.match(/export (?:const|function|interface|type) (\w+)/g) || [];
    const publicAPI = ["showHelp", "showVersion", "showUsage", "HELP_TEXT", "APP_NAME", "VERSION", "HelpTextConfig"];
  ...
  await t.step("provides clear and focused public API", async () => {
    const { showHelp, showVersion, showUsage, HELP_TEXT, APP_NAME, VERSION, ...otherExports } = await import("./help.ts");
    ...
    const exportedKeys = Object.keys({ showHelp, showVersion, showUsage, HELP_TEXT, APP_NAME, VERSION, ...otherExports });
