...
import type { ValidatedParams, PromptGeneratorError } from "./two_params_prompt_generator.ts";

Deno.test("Architecture: TwoParamsPromptGenerator follows single responsibility principle", () => {
  ...
  assertExists(generator.generatePrompt, "Should have generatePrompt method");
  ...
  const publicMethods = methods.filter(name => !name.startsWith("create") && !name.startsWith("validate") && !name.startsWith("build") && !name.startsWith("generate") || name === "generatePrompt");
...
Deno.test("Architecture: TwoParamsPromptGenerator error types follow discriminated union pattern", () => {
...
Deno.test("Architecture: TwoParamsPromptGenerator dependencies follow correct direction", () => {
...
Deno.test("Architecture: ValidatedParams interface enforces type safety", () => {
...
Deno.test("Architecture: TwoParamsPromptGenerator maintains clear separation of concerns", () => {
