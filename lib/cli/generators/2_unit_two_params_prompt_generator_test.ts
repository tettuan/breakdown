...
const logger = new BreakdownLogger("two-params-prompt-generator-test");
...
Deno.test("Unit: TwoParamsPromptGenerator generates prompt successfully with valid inputs", async () => {
  ...
  logger.debug("Prompt generation result", { success: result.ok });
  ...
  if (!result.ok) {
    ...
    logger.debug("Expected failure in test environment", { errorKind: result.error.kind });
  }
});
...
Deno.test("Unit: TwoParamsPromptGenerator handles invalid configuration", async () => {
  ...
  if (!result.ok) {
    assertEquals(result.error.kind, "InvalidConfiguration", "Should be InvalidConfiguration error");
...
Deno.test("Unit: TwoParamsPromptGenerator handles missing required configuration properties", async () => {
  ...
  if (!result.ok) {
    ...
    assertEquals(result.error.kind, "ConfigurationValidationError", "Should be ConfigurationValidationError due to missing config");
    if (result.error.kind === "ConfigurationValidationError") {
      ...
      assertEquals(Array.isArray(result.error.missingProperties), true, "Missing properties should be array");
    }
  }
});
...
Deno.test("Unit: TwoParamsPromptGenerator processes custom variables correctly", async () => {
  ...
  if (!result.ok) {
    logger.debug("Custom variables test", { 
...
Deno.test("Unit: TwoParamsPromptGenerator handles factory creation errors", async () => {
  ...
  if (!result.ok) {
    logger.debug("Factory creation error test", { errorKind: result.error.kind });
...
Deno.test("Unit: TwoParamsPromptGenerator handles all demonstrative types", async () => {
  ...
  for (const demonstrativeType of demonstrativeTypes) {
    for (const layerType of layerTypes) {
      const validatedParams: ValidatedParams = {
        demonstrativeType: demonstrativeType as any,
        layerType
      };
...
Deno.test("Unit: TwoParamsPromptGenerator respects verbose flag", async () => {
  ...
  // Test with verbose
  const resultVerbose = await generator.generatePrompt(
    configVerbose,
    validatedParams,
    {},
    { standardVariables: {}, customVariables: {}, allVariables: {} }
  ...
  const resultQuiet = await generator.generatePrompt(
    configQuiet,
    validatedParams,
    {},
    { standardVariables: {}, customVariables: {}, allVariables: {} }
