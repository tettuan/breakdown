...
async function loadConfig(configPath: string): Promise<CustomConfig> {
  try {
    ...
    return parse(content) as CustomConfig;
  } catch (error: unknown) {
...
Deno.test("BreakdownConfig - CustomConfig Integration", async (t) => {
...
Deno.test("BreakdownConfig - Custom Config Validation", async (t) => {
