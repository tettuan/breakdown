...
Deno.test("Help Module Architecture", async (t) => {
  ...
  await t.step("has minimal and appropriate dependencies", async () => {
    ...
    assertEquals(helpContent.includes('from "../version.ts"'), true, "Should only import from version.ts");
  ...
  await t.step("maintains proper dependency direction", async () => {
    ...
    assertEquals(helpContent.includes('from "../../'), false, "Should not import from parent directories");
    assertEquals(helpContent.includes('from "../factory'), false, "Should not depend on factory layer");
    assertEquals(helpContent.includes('from "../types'), false, "Should not depend on types directly");
  ...
  await t.step("defines clear module boundaries", async () => {
    const { showHelp, showVersion, showUsage, HELP_TEXT, VERSION, APP_NAME, ...otherExports } = await import("./help.ts");
    ...
    assertEquals((otherExports as any).generateHelpText, undefined, "Should not export internal generateHelpText");
    assertEquals((otherExports as any).DEFAULT_HELP_CONFIG, undefined, "Should not export internal config");
  });
