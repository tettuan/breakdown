...
Deno.test("UnifiedFactory - 1_behavior - creates factory with valid configuration", async () => {
  ...
  try {
    ...
    assertEquals(factory instanceof UnifiedPromptVariablesFactory, true, "Should be UnifiedPromptVariablesFactory instance");
...
Deno.test("UnifiedFactory - 1_behavior - handles configuration loading failures gracefully", async () => {
  ...
  try {
    ...
    logger.debug("Factory failed with expected error", { error: error instanceof Error ? error.message : String(error) });
  }
});
...
Deno.test("LegacyAdapter - 1_behavior - converts legacy parameters to unified format", async () => {
  ...
  try {
    ...
    // Verify types
    assertEquals(unifiedParams.directive instanceof DirectiveType, true, "Directive should be DirectiveType");
  ...
  try {
    ...
    if (factoryResult.ok) {
      ...
      const requiredProperties = [
        "promptFilePath", "inputFilePath", "outputFilePath", 
        "schemaFilePath", "directive", "layer"
      ];
