...
describe("OutputFilePathResolver - Class Structure", () => {
  it("should have a clear single responsibility for output path resolution", () => {
    ...
    if ((resolver as any).readFile) { assertEquals(typeof (resolver as any).readFile, "function"); }
    if ((resolver as any).writeFile) { assertEquals(typeof (resolver as any).writeFile, "function"); }
    if ((resolver as any).processContent) { assertEquals(typeof (resolver as any).processContent, "function"); }
  });
  ...
  it("should properly encapsulate internal logic", () => {
    ...
    if ((resolver as any).getLayerType) { assertEquals(typeof (resolver as any).getLayerType, "function"); }
  ...
  it("should maintain immutable state after construction", () => {
    ...
    assertExists(path1); assertExists(path2);
  });
});
