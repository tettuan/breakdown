...
import { 
  ...
  type StdinVariableFactoryError
...
Deno.test("StdinVariableFactory - 1_behavior - validates source parameter", () => {
  ...
  if (!invalidResult.ok) {
    assertEquals(invalidResult.error.kind, "InvalidStdinSource", "Should be InvalidStdinSource error");
  }
});
...
Deno.test("StdinVariableFactory - 1_behavior - createFromText convenience method works", () => {
  ...
  if (result1.ok) {
    assertEquals(result1.data.toRecord().input_text, "Hello from text", "Should contain correct text");
  }
  ...
  if (result2.ok) {
    assertEquals(result2.data.toRecord().input_text, "Hello from file", "Should contain correct text");
  }
});
...
Deno.test("StdinVariableFactory - 1_behavior - default instance is available", () => {
  ...
  if (result.ok) {
    assertEquals(result.data.toRecord().input_text, "Test with default instance", 
