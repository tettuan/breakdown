...
Deno.test("Legacy Methods - 1_behavior - legacy methods string template is valid", () => {
  ...
  assertEquals(
    legacyMethods.includes("public get promptPath()"),
    true,
    "Should contain promptPath getter",
  );
  assertEquals(
    legacyMethods.includes("public get schemaPath()"),
    true,
    "Should contain schemaPath getter",
  );
  assertEquals(
    legacyMethods.includes("public getDirective()"),
    true,
    "Should contain getDirective method",
  );
  assertEquals(
    legacyMethods.includes("public getLayerType()"),
    true,
    "Should contain getLayerType method",
  );
});
...
Deno.test("Legacy Methods - 1_behavior - build method returns Promise<void>", () => {
  ...
  assertEquals(
    legacyMethods.includes("return Promise.resolve();"),
    true,
    "Build should return Promise.resolve()",
  );
});
...
Deno.test("Legacy Methods - 1_behavior - property getters access correct fields", () => {
  ...
  assertEquals(
    legacyMethods.includes("return this.promptFilePath;"),
    true,
    "promptPath should return promptFilePath",
  );

  // Verify schemaPath getter accesses schemaFilePath
  assertEquals(
    legacyMethods.includes("return this.schemaFilePath;"),
    true,
    "schemaPath should return schemaFilePath",
  );

  // Verify getDirective accesses demonstrativeType
  assertEquals(
    legacyMethods.includes("return this.cliParams.demonstrativeType;"),
    true,
    "getDirective should return demonstrativeType",
  );

  // Verify getLayerType accesses layerType
  assertEquals(
    legacyMethods.includes("return this.cliParams.layerType;"),
    true,
    "getLayerType should return layerType",
  );
});
...
Deno.test("Legacy Methods - 1_behavior - string template can be evaluated safely", () => {
  ...
  for (const pattern of dangerousPatterns) {
    assertEquals(
      legacyMethods.includes(pattern),
      false,
      `Should not contain dangerous pattern: ${pattern}`,
    );
  }
