...
Deno.test("PathResolver Architecture", async (t) => {
  ...
  await t.step("implements safe path resolution patterns", async () => {
    try {
      const promptResolverContent = await Deno.readTextFile("./lib/factory/prompt_template_path_resolver.ts");
      
      // Should use safe path operations
      assertEquals(promptResolverContent.includes("join("), true, "Should use path.join for safety");
      assertEquals(promptResolverContent.includes("resolve("), true, "Should use path.resolve for normalization");
      
      // Should validate paths before use
      assertEquals(promptResolverContent.includes("isAbsolute"), true, "Should check for absolute paths");
      assertEquals(promptResolverContent.includes("existsSync"), true, "Should verify file existence");
      
      // Should not use dangerous path operations
      assertEquals(promptResolverContent.includes(".."), false, "Should not use relative path traversal");
      assertEquals(promptResolverContent.includes("eval("), false, "Should not use eval");
      assertEquals(promptResolverContent.includes("require("), false, "Should not use dynamic require");
      
    } catch (error) {
      console.warn(`Prompt resolver test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  await t.step("follows Result type pattern for error handling", async () => {
    try {
      ...
      console.warn(`PromptTemplatePathResolver import test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  await t.step("restricts file system access to configured directories", async () => {
    try {
      ...
      assertEquals(content.includes("DEFAULT_PROMPT_BASE_DIR"), true, "Should use configured base directory");
      
      // Should not access system directories
      const forbiddenPaths = ["/etc/", "/usr/", "/var/", "/tmp/", "C:\\", "/root/"];
      for (const path of forbiddenPaths) {
        assertEquals(content.includes(`"${path}"`), false, `Should not access system path: ${path}`);
      }
      
      // Should not use environment variables for paths (security)
      assertEquals(content.includes("Deno.env"), false, "Should not use environment variables for paths");
      assertEquals(content.includes("process.env"), false, "Should not use process.env");
      
    } catch (error) {
      console.warn(`File system access test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  await t.step("implements proper abstraction boundaries", async () => {
    ...
    for (const file of resolverFiles) {
      try {
        ...
        assertEquals(content.includes("../cli/handlers"), false, "Should not depend on CLI handlers");
        assertEquals(content.includes("../cli/processors"), false, "Should not depend on CLI processors");
        
        // Should not handle command-line parsing
        assertEquals(content.includes("Deno.args"), false, "Should not access command line arguments");
        assertEquals(content.includes("process.argv"), false, "Should not access process arguments");
        
        // Should focus on path resolution only
        assertEquals(content.includes("console.log"), false, "Should not handle output directly");
        
      } catch (error) {
        console.warn(`Skipping abstraction test for ${file}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  });
  ...
  await t.step("enforces immutable configuration usage", async () => {
    try {
      ...
      assertEquals(content.includes("Object.assign(config"), false, "Should not modify config objects");
      assertEquals(content.includes("delete config"), false, "Should not delete config properties");
      
    } catch (error) {
      console.warn(`Configuration immutability test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
});
...
Deno.test("PathResolver Security Architecture", async (t) => {
  ...
  await t.step("prevents path traversal vulnerabilities", async () => {
    try {
      ...
      for (const pattern of dangerousPatterns) {
        ...
        const directUsage = new RegExp(`["'\`].*${pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*["'\`]`);
        assertEquals(directUsage.test(content), false, 
          `Should not directly use dangerous path pattern: ${pattern}`);
      }
      
    } catch (error) {
      console.warn(`Path traversal test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  await t.step("validates input parameters", async () => {
    try {
      ...
      console.warn(`Input validation test skipped: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  await t.step("limits file system operations to read-only", async () => {
    ...
    for (const file of resolverFiles) {
      try {
        ...
        console.warn(`Read-only test skipped for ${file}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  });
});