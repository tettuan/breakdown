...
import { assertEquals, assertExists, assertInstanceOf as _assertInstanceOf, assert } from "@std/assert";
...
describe("PromptVariablesFactory Structure - Class Design Principles", () => {
  it("should adhere to single responsibility principle", async () => {
    ...
    if (typesResult.ok) {
      ...
      assertEquals(typeof (factory as any).promptPathResolver, "object");
      assertEquals(typeof (factory as any).inputPathResolver, "object");
      assertEquals(typeof (factory as any).outputPathResolver, "object");
      assertEquals(typeof (factory as any).schemaPathResolver, "object");
    }
  });
  ...
  it("should properly encapsulate internal state and dependencies", async () => {
    ...
    if (typesResult.ok) {
      ...
      assertEquals(typeof (factory as any).config, "object");
      assertEquals(typeof (factory as any).baseDirOverride, "undefined");
...
describe("PromptVariablesFactory Structure - Interface Consistency", () => {
  ...
  it("should support consistent factory configuration patterns", () => {
    ...
    if (typesResult.ok) {
      ...
      const totalityFactory = TotalityPromptVariablesFactory.createWithConfig(baseConfig, totalityParams);
...
describe("PromptVariablesFactory Structure - Dependency Management", () => {
  it("should properly inject and manage path resolver dependencies", async () => {
    ...
    if (typesResult.ok) {
      ...
      const promptResolver = (factory as any).promptPathResolver;
      const inputResolver = (factory as any).inputPathResolver;
      const outputResolver = (factory as any).outputPathResolver;
      const schemaResolver = (factory as any).schemaPathResolver;
  ...
  it("should handle configuration dependency properly", async () => {
    ...
    if (typesResult.ok) {
      ...
      const internalConfig = (factory as any).config;
      assertExists(internalConfig);
      assertEquals(internalConfig.app_prompt.base_dir, "custom_prompts");
      assertEquals(internalConfig.app_schema.base_dir, "custom_schemas");
      assertEquals(internalConfig.custom_setting, "test_value");
  ...
  it("should manage type conversion between legacy and Totality systems", async () => {
    ...
    if (typesResult.ok) {
      ...
      const legacyParams = (factory as any).legacyParams;
      if (legacyParams) {
