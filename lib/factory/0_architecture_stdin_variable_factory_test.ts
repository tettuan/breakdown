...
import { StdinVariableFactory, defaultStdinVariableFactory } from "./stdin_variable_factory.ts";
...
Deno.test("0_architecture: Domain boundary constraint - no exceptions across boundary", () => {
  ...
  for (const { desc, input } of boundaryViolationTests) {
    ...
    try {
      ...
      assertEquals(false, true, `Boundary violation: exception thrown for ${desc}: ${errorMessage}`);
    }
  }
});
