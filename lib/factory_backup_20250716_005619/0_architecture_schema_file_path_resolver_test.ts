...
import { createWholeTwoParamsResult } from "../types/two_params_result_extension.ts";
...
Deno.test("SchemaFilePathResolver Architecture - Smart Constructor enforces validation", () => {
  ...
  const invalidParams = [
    ...
    "string",
    123,
    { directiveType: "", layerType: "project", demonstrativeType: "", options: {} },
    { directiveType: "to", layerType: "", demonstrativeType: "to", options: {} },
    { directiveType: "  ", layerType: "project", demonstrativeType: "  ", options: {} },
    { directiveType: "to", layerType: "  ", demonstrativeType: "to", options: {} },
  ];
...
Deno.test("SchemaFilePathResolver Architecture - Immutability through deep copy", () => {
  ...
  const originalBaseDirResult = result.data.resolveSafe();
  assert(originalBaseDirResult.ok, "Should resolve base dir successfully");
  const originalBaseDir = originalBaseDirResult.data;
  ...
  const currentBaseDirResult = result.data.resolveSafe();
  assert(currentBaseDirResult.ok, "Should resolve base dir successfully");
  const currentBaseDir = currentBaseDirResult.data;
...
Deno.test("SchemaFilePathResolver Architecture - Result type totality (no exceptions)", () => {
  ...
  const baseDirResult = result.data.resolveSafe();
  assert(baseDirResult.ok, "resolveSafe should return successful Result");
  assertEquals(typeof baseDirResult.data, "string", "resolveSafe should return string in Result");

  // getPath returns Result
  const pathResult2 = result.data.getPath();
  assert("ok" in pathResult2, "getPath should return Result type");
});
...
Deno.test("SchemaFilePathResolver Architecture - Error discrimination with type guards", () => {
  const errors: SchemaPathError[] = [
    { kind: "SchemaNotFound", path: "/test", attempted: ["/test"] },
    { kind: "InvalidSchemaPath", path: "/test", reason: "test error" },
    { kind: "SchemaValidationFailed", details: "test error" },
  ];

  // Test type guards - commented out as functions are not exported
  ...
  for (const error of errors) {
    // SchemaPathError has different structure than PathResolutionError
    // We just verify the structure is correct
    assert(error.kind.length > 0, "Error should have non-empty kind");
    assert(typeof error === "object", "Error should be an object");
  }
});
...
Deno.test("SchemaFilePathResolver Architecture - Backward compatibility maintained", () => {
  ...
  const newParams: TwoParams_Result = createWholeTwoParamsResult({
    type: "two",
    params: ["to", "project"],
    demonstrativeType: "to",
    layerType: "project",
  ...
  if (legacyResult.ok) {
    const baseDirResult = legacyResult.data.resolveSafe();
    assert(baseDirResult.ok && typeof baseDirResult.data === "string", "resolveSafe should work");

    const stringResult = legacyResult.data.getPathAsStringSafe();
    assert("ok" in stringResult, "getPathAsStringSafe should return Result");

    // getPathAsStringSafe should return error for non-existent directory
    ...
    if (errorResult.ok) {
      const pathResult = errorResult.data.getPathAsStringSafe();
      assert(!pathResult.ok, "Should return error for non-existent directory");
    }
  }
});
...
Deno.test("SchemaFilePathResolver Architecture - Exhaustive error handling", () => {
  ...
  const testCases: Array<{
    ...
    {
      config: null,
      params: { directiveType: "to", layerType: "project", demonstrativeType: "to", options: {} },
    ...
    {
      config: { app_schema: { base_dir: "/test" } },
      params: { directiveType: "", layerType: "project", demonstrativeType: "", options: {} },
      expectedError: "InvalidParameterCombination",
    },
    {
      config: { app_schema: { base_dir: "/test" } },
      params: { directiveType: "to", layerType: "", demonstrativeType: "to", options: {} },
      expectedError: "InvalidParameterCombination",
    },
  ];
