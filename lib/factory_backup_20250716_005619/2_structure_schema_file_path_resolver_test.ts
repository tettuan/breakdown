...
Deno.test("SchemaFilePathResolver Structure - Parameter structures support both formats", () => {
  ...
  const mixedParams = createWholeTwoParamsResult({
    type: "two",
    params: ["defect", "task"],
...
Deno.test("SchemaFilePathResolver Structure - Error type discrimination structure", () => {
  ...
  const schemaNotFound: SchemaPathError = {
    kind: "SchemaNotFound",
  ...
  // assert(isInvalidParametersError(invalidParams)); // Function not exported
  assertEquals(invalidParams.directiveType, "invalid");
  assertEquals(invalidParams.layerType, "type");

  const configError: SchemaPathError = {
...
Deno.test("SchemaFilePathResolver Structure - PathResolutionError to SchemaPathError mapping", () => {
  const mappings: Array<{
    input: PathResolutionError;
    expectedKind: SchemaPathError["kind"];
    checkProperties?: (error: SchemaPathError) => void;
  }> = [
    {
    ...
    },
    {
      ...
      checkProperties: (error) => {
        if (error.kind === "InvalidParameters") {
          assertEquals((error as any).directiveType, "bad");
          assertEquals((error as any).layerType, "params");
        }
      },
    },
    {
...
Deno.test("SchemaFilePathResolver Structure - Error formatting completeness", () => {
  ...
  // Test all SchemaPathError kinds format correctly
  const schemaErrors: SchemaPathError[] = [
  ...
  for (const error of schemaErrors) {
    ...
    switch (error.kind) {
      ...
      case "InvalidParameters":
        assert(formatted.includes((error as any).directiveType));
        assert(formatted.includes((error as any).layerType));
        break;
