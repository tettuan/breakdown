...
const _validMinimalInput: StdinFactoryInput = {
...
Deno.test("0_architecture: Domain boundary constraint - no exceptions across boundary", () => {
  ...
  for (const { desc, input } of boundaryViolationTests) {
    ...
    try {
      const result = factory.create((input as unknown) as StdinFactoryInput);
...
Deno.test("0_architecture: Error type consistency", () => {
  ...
  const errorCases = [
    ...
    {
      input: { inputText: "test", source: "invalid" as unknown as "file" | "pipe" | "cli" },
      expectedKind: "InvalidStdinSource",
    },
  ];

  for (const { input, expectedKind: _expectedKind } of errorCases) {
