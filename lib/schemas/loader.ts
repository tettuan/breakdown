...
export async function loadSchema(type: string, layer: string): Promise<unknown> {
  ...
  if (USE_EMBEDDED_RESOURCES) {
    if (!SCHEMAS[type as keyof typeof SCHEMAS] || !SCHEMAS[type as keyof typeof SCHEMAS][layer as string]) {
      throw new Error(`Schema not found for ${type}/${layer}`);
    }
    return JSON.parse(SCHEMAS[type as keyof typeof SCHEMAS][layer as string]);
  }
  
  // Otherwise, load from file system
  const config = getConfig();
  const schemaPath = join(config.working_directory, "schemas", type, layer, "base.schema.json");
