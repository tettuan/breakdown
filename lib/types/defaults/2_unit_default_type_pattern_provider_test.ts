...
Deno.test("2_unit - DefaultTypePatternProvider.getDefaultConfig returns defaultConfigTwoParams", () => {
  ...
  // 同一オブジェクトを返すことを確認
  assertEquals(config, defaultConfigTwoParams, "Should return defaultConfigTwoParams");
...
Deno.test("2_unit - DefaultTypePatternProvider.debug returns structured debug info", () => {
  ...
  // 必須フィールドの確認
  assertEquals(debugInfo.providerType, "DefaultTypePatternProvider", "Should identify provider type");
  assertEquals(debugInfo.directivePattern, "^(to|summary|defect)$", "Should include directive pattern");
  assertEquals(debugInfo.layerPattern, "^(project|issue|task)$", "Should include layer pattern");
  
  // 有効値リストの確認
  assertEquals(Array.isArray(debugInfo.validDirectives), true, "Should include directive values array");
  assertEquals(debugInfo.validDirectives.length, 3, "Should have 3 directive values");
  assertEquals(debugInfo.validDirectives, ["to", "summary", "defect"], "Directive values should match");
