...
Deno.test("defaultConfigTwoParams - Cross-Domain Consistency", async (t) => {
  ...
  await t.step("should be consistent with validation system expectations", () => {
    ...
    expectedValueOptions.forEach((option) => {
      assertEquals(
        validation.allowedValueOptions.includes(option),
        true,
    ...
    assertEquals(
      validation.allowedFlagOptions.length,
      0,
      "Should not have flag options by default",
    );
  });
});
...
Deno.test("defaultConfigTwoParams - Serialization Consistency", async (t) => {
  await t.step("should serialize and deserialize consistently", () => {
    ...
    assertEquals(
      deserialized.params.two.directiveType.pattern,
      original.params.two.directiveType.pattern,
    );
    assertEquals(deserialized.params.two.layerType.pattern, original.params.two.layerType.pattern);
    assertEquals(
      deserialized.params.two.validation.allowedFlagOptions,
      original.params.two.validation.allowedFlagOptions,
    );
    assertEquals(
      deserialized.params.two.validation.allowedValueOptions,
      original.params.two.validation.allowedValueOptions,
    );
    assertEquals(
      deserialized.params.two.validation.userVariableOption,
      original.params.two.validation.userVariableOption,
    );
    assertEquals(
      deserialized.params.two.validation.stdinAllowed,
      original.params.two.validation.stdinAllowed,
    );
  });
...
Deno.test("defaultConfigTwoParams - Schema Validation", async (t) => {
  ...
  await t.step("should validate array option consistency", () => {
    ...
    assertEquals(
      flagOptions.length,
      new Set(flagOptions).size,
      "allowedFlagOptions should not have duplicates",
    );
    assertEquals(
      valueOptions.length,
      new Set(valueOptions).size,
      "allowedValueOptions should not have duplicates",
    );

    // Test array options don't contain empty strings
    if (flagOptions.length > 0) {
      assertEquals(
        flagOptions.every((opt) => opt.length > 0),
        true,
        "allowedFlagOptions should not contain empty strings",
      );
    }
    assertEquals(
      valueOptions.every((opt) => opt.length > 0),
      true,
      "allowedValueOptions should not contain empty strings",
    );
  });
});
