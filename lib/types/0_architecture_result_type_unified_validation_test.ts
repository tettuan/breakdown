...
import { assertEquals, assertExists, assertInstanceOf as _assertInstanceOf } from "../deps.ts";
...
Deno.test("Result Type - Performance Validation", async (t) => {
  ...
  await t.step("should have efficient utility function performance", () => {
    const results: Result<number, string>[] = Array.from(
      { length: 1000 },
      (_, i) => i % 3 === 0 ? error(`Error ${i}`) : ok(i),
    );
