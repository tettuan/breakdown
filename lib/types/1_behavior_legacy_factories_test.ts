...
Deno.test("Legacy Factories Behavior - Type guard accuracy", () => {
  ...
  for (const { type, expectedGuard } of demonstrativeTypes) {
    ...
    const correctResult =
      (DemonstrativeTypeGuards as Record<string, (type: DemonstrativeType) => boolean>)
        [expectedGuard](type);
  ...
  for (const { type, expectedGuard } of layerTypes) {
    // Test correct guard returns true
    const correctResult =
      (LegacyLayerTypeGuards as Record<string, (type: LegacyLayerType) => boolean>)[expectedGuard](
        type,
      );
    assertEquals(correctResult, true, `${expectedGuard} should return true for matching type`);
    ...
    for (const otherGuard of otherGuards) {
      const otherResult =
        (LegacyLayerTypeGuards as Record<string, (type: LegacyLayerType) => boolean>)[otherGuard](
          type,
        );
      assertEquals(otherResult, false, `${otherGuard} should return false for ${type.kind} type`);
    }
...
Deno.test("Legacy Factories Behavior - Alias factory equivalence", () => {
  ...
  for (const { method, expected } of directiveAliasTests) {
    const originalResult =
      (DemonstrativeTypeFactory as unknown as Record<string, () => DemonstrativeType>)[method]();
    const aliasResult = (DirectiveFactory as unknown as Record<string, () => DemonstrativeType>)
      [method]();
  ...
  for (const { method, expected } of layerAliasTests) {
    const originalResult =
      (LegacyLayerTypeFactory as unknown as Record<string, () => LegacyLayerType>)[method]();
    const aliasResult = (LayerFactory as unknown as Record<string, () => LegacyLayerType>)
      [method]();
