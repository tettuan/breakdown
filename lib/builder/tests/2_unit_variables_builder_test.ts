...
Deno.test("VariablesBuilder - empty value error", () => {
  ...
  if (!result.ok) {
    ...
    const emptyValueErrors = result.error.filter(err => err.kind === "EmptyValue");
...
Deno.test("VariablesBuilder - accumulates multiple errors", () => {
  ...
  if (!result.ok) {
    ...
    assertExists(result.error.find((e) => e.kind === "InvalidName"));
    assertExists(result.error.find((e) => e.kind === "EmptyValue"));
    assertExists(result.error.find((e) => e.kind === "InvalidPrefix"));
  }
});
...
Deno.test("VariablesBuilder - method chaining works correctly", () => {
  ...
  assertEquals(builder.getVariableCount(), 5);
  assertEquals(builder.getErrorCount(), 0);
});
...
Deno.test("VariablesBuilder - cross-type duplicate checking", () => {
  ...
  builder.addStdinVariable("some text");

  // Try to add user variable with same name
  builder.addUserVariable("input_text", "different text");
