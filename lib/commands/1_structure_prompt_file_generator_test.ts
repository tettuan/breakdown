...
describe("Structure: Data Flow Patterns", () => {
  ...
  it("should handle stdin input flow separately", () => {
    ...
    const stdinErrorPattern = /type:\s*PromptFileErrorType\.InputFileNotFound.*No input provided via stdin/s;
    assertEquals(
...
describe("Structure: Template Processing Integration", () => {
  it("should structure adapter creation and usage", () => {
    ...
    assertEquals(
      methodString.includes("const adapter ="),
      true,
    ...
    assertEquals(
      methodString.includes("await adapter.validateAndGenerate()"),
      true,
  ...
  it("should handle adapter results properly", () => {
    ...
    assertEquals(
      methodString.includes("const result = await adapter.validateAndGenerate()"),
      true,
...
describe("Structure: CommandResult Interface Compliance", () => {
  it("should return CommandResult with all required fields", async () => {
    ...
    const returnPattern = /return\s*{\s*success:\s*(true|false),\s*output:\s*[^,}]+,\s*error:\s*[^}]+}/g;
