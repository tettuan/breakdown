    ...
    try {
      ...
      try {
        ...
        if (templateError) {
          ...
          assertEquals(
            templateError.canAutoResolve,
            false,
            "Permission errors cannot be auto-resolved",
          );
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        ...
        "Should preserve non-template errors",
      );
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        templateError.canAutoResolve,
        true,
        "Invalid templates should be auto-resolvable",
      );

      // Should suggest template fixes
      assertExists(
        templateError.suggestions.find((s) =>
