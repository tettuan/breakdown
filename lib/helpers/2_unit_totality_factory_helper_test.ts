...
Deno.test("TotalityFactoryHelper - Unit: createTotalityFactory - default options", async () => {
  ...
  if (result.ok) {
    ...
    assertEquals(typeof bundle.createPromptFactory, "function", "Should provide createPromptFactory");
  } else {
...
Deno.test("TotalityFactoryHelper - Unit: validateConfigurationPatterns - custom config", async () => {
  ...
  if (!result.valid) {
    assertEquals(result.details.some(d => d.includes("failed")), true, "Should indicate failure in details");
  }
});
...
Deno.test("TotalityFactoryHelper - Unit: createTotalityPromptFactory - invalid parameters", async () => {
  ...
  if (!result.ok) {
    assertEquals(typeof result.error, "string", "Should provide error message");
    assertEquals(result.error.includes("Failed") || result.error.includes("BreakdownConfig") || result.error.includes("validation") || result.error.includes("Invalid"), true, "Error should mention failure or configuration issue");
  }
});
...
Deno.test("TotalityFactoryHelper - Unit: Factory bundle component functionality", async () => {
  ...
  if (result.ok) {
    ...
    if (typesResult.ok) {
      ...
      const promptFactory = await bundle.createPromptFactory(params);
