...
Deno.test("Unit: TemplateErrorHandler detects file not found errors", () => {
  ...
  for (const testCase of testCases) {
    ...
    if (testCase.context?.templatePath) {
      assertEquals(result?.templatePath, testCase.context.templatePath, "Should preserve template path");
    }
  }
});
...
Deno.test("Unit: TemplateError generates detailed messages correctly", () => {
  ...
  assertStringIncludes(message, "Template: /prompts/complex/template.md", "Should include template path");
  ...
  assertStringIncludes(message, "- Validate variable references", "Should include second suggestion");
  assertStringIncludes(message, "- Test with simple content", "Should include third suggestion");
  assertStringIncludes(message, "🔧 Auto-resolution available", "Should include auto-resolution info");
});
