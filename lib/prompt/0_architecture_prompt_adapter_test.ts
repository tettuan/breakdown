...
const logger = new BreakdownLogger("architecture-prompt-adapter");

describe("PromptAdapter Architecture - Module Dependencies", () => {
  it("should only depend on allowed external packages", async () => {
    logger.debug("Testing external package dependencies");
    
    const fileContent = await Deno.readTextFile("lib/prompt/prompt_adapter.ts");
    const imports = fileContent.match(/import.*from\s+["']([^"']+)["']/g) || [];
    
    const externalImports = imports
      .map(imp => imp.match(/from\s+["']([^"']+)["']/)?.[1])
      .filter(Boolean)
      .filter(imp => imp?.startsWith("@") || imp?.startsWith("jsr:") || imp?.startsWith("https://"));
  ...
  it("should maintain proper layer separation", async () => {
    logger.debug("Testing layer separation");
  ...
  it("should expose only necessary public interfaces", async () => {
    logger.debug("Testing public interface exposure");
...
describe("PromptAdapter Architecture - Adapter Pattern Compliance", () => {
  it("should follow adapter pattern responsibilities", async () => {
    ...
    adapterMethods.forEach(method => {
      const hasMethod = fileContent.includes(`async ${method}`) || fileContent.includes(`${method}(`);
    ...
    delegationTargets.forEach(target => {
      const hasDelegation = fileContent.includes(`new ${target}`) || fileContent.includes(`${target}.`);
...
describe("PromptAdapter Architecture - Error Handling Consistency", () => {
  ...
  it("should validate all required dependencies exist", () => {
    logger.debug("Testing dependency existence");
