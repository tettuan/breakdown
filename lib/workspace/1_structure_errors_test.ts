...
const logger = new BreakdownLogger("test-structure-errors");
...
describe("Workspace Errors - Structure Tests", () => {
  describe("Single Responsibility Principle", () => {
    it("should have each error class handle a single concern", async () => {
      ...
      for (const { ErrorClass, responsibility, validMessages } of errorResponsibilities) {
        ...
        for (const message of validMessages) {
          ...
          const expectedName = responsibilityKey === "initialization" ? "init" : 
                              responsibilityKey === "configuration" ? "config" : 
                              responsibilityKey === "path" ? "path" : 
                              responsibilityKey === "directory" ? "directory" : responsibilityKey;
          assertEquals(error.constructor.name.toLowerCase().includes(expectedName), true);
          
          logger.debug("Responsibility verified", {
      ...
      logger.debug("Single responsibility principle verified", {
    ...
    it("should not have overlapping responsibilities", async () => {
      logger.debug("Testing responsibility boundaries", {
        testType: "structure",
      ...
      for (const { scenario, primaryError, notError, reason } of scenarios) {
        ...
        logger.debug("Responsibility boundary verified", {
          scenario,
      ...
      logger.debug("No overlapping responsibilities detected", {
  ...
  describe("Class cohesion and coupling", () => {
    it("should have high cohesion within each error class", () => {
      logger.debug("Testing class cohesion", {
        testType: "structure",
        aspect: "cohesion"
      });
      ...
      for (const error of errorInstances) {
        ...
        assertEquals(codePrefix.includes(namePrefix) || namePrefix.includes(codePrefix) || codePrefix === namePrefix, true);
        
        logger.debug("Cohesion verified", {
          errorType: error.name,
          propertiesRelated: true
        });
      }

      logger.debug("High cohesion confirmed", {
    ...
    it("should have loose coupling between error classes", () => {
      logger.debug("Testing class coupling", {
        testType: "structure",
        aspect: "coupling"
      });
      ...
      for (const ErrorClass of errorClasses) {
        ...
        logger.debug("Low coupling verified", {
          class: ErrorClass.name,
          noExternalDependencies: true
        });
      }

      logger.debug("Loose coupling confirmed", {
  ...
  describe("Abstraction levels", () => {
    it("should maintain consistent abstraction levels", () => {
      logger.debug("Testing abstraction consistency", {
        testType: "structure",
      ...
      for (const { error, expectedCode } of derivedErrors) {
        ...
        logger.debug("Abstraction level verified", {
          errorType: error.name,
          concreteImplementation: true
        });
      }

      logger.debug("Abstraction levels are consistent", {
    ...
    it("should not mix abstraction levels within classes", () => {
      logger.debug("Testing abstraction level separation", {
        testType: "structure",
      ...
      for (const { error, abstractionLevel, shouldNotContain } of testCases) {
        ...
        logger.debug("Abstraction purity verified", {
          errorType: error.name,
          abstractionLevel,
          noMixing: true
        });
      ...
      logger.debug("Abstraction levels are not mixed", {
  ...
  describe("Error information structure", () => {
    it("should structure error information consistently", async () => {
      ...
      for (const { ErrorClass, message, expectedStructure } of errorCases) {
        ...
        logger.debug("Information structure verified", {
          errorType: ErrorClass.name,
          structureComplete: true
        });
      }

      logger.debug("Error information consistently structured", {
    ...
    it("should provide sufficient context in error messages", async () => {
      logger.debug("Testing error context sufficiency", {
        testType: "structure",
      ...
      const contextualErrors = [
        {
          ErrorClass: WorkspaceInitError,
          contexts: [
            { message: "Failed to create directory: Permission denied at /workspace", hasPath: true },
        ...
        },
        {
          ErrorClass: WorkspacePathError,
          contexts: [
            { message: "Invalid path: '../../../etc/passwd' attempts to escape workspace", hasSecurity: true },
      ...
      for (const { ErrorClass, contexts } of contextualErrors) {
        for (const context of contexts) {
          ...
          logger.debug("Context sufficiency verified", {
            errorType: ErrorClass.name,
      ...
      logger.debug("All errors provide sufficient context", {
  ...
  describe("Extensibility and maintainability", () => {
    it("should be easily extensible for new error types", async () => {
      logger.debug("Testing extensibility", {
        testType: "structure",
      ...
      logger.debug("Extensibility verified", {
    ...
    it("should maintain clear error categorization", async () => {
      logger.debug("Testing error categorization", {
        testType: "structure",
      ...
      for (const [category, ErrorClasses] of Object.entries(errorCategories)) {
        for (const ErrorClass of ErrorClasses) {
          ...
          logger.debug("Categorization verified", {
            category,
            errorType: ErrorClass.name,
      ...
      logger.debug("Error categorization is clear", {
