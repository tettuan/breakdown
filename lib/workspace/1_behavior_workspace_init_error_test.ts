...
Deno.test("Behavior: Error Details Propagation", () => {
  ...
  const metadata = complexError.details?.metadata as Record<string, unknown>;
  assertEquals(metadata?.user, "testuser");
...
Deno.test("Behavior: Factory Function Edge Cases", () => {
  ...
  const unknownError = createWorkspaceInitError(
    "unknown" as "directory" | "config" | "location" | "exists",
    "/test/path",
  );
