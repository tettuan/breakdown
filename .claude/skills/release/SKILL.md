...
全リリースフロー（version bump → CI → PR → merge → vtag → JSR publish）を管理する。CI green なら自動で次ステップへ進む（ユーザー確認不要）。
...
- 各ステップは**実行 → 結果確認 → 自動で次へ**。CI/コマンド成功なら確認待ちせずに進める。
- 前ステップが未完了（`[ ]`）の状態で次ステップへ進まない。
- 失敗時は該当ステップに留まり、原因を解消してから再実行（次へ進まない）。失敗・想定外の状態（branch divergence, conflict, CI failure 等）はユーザーに報告する。
...
- [ ] **Step 1. Prepare release branch** — `git fetch origin && git checkout develop && git pull --ff-only origin develop && git checkout -b release/vX.Y.Z`
  - `pull --ff-only` が失敗した場合（local が diverged）は `git reset --hard origin/develop` でリセットしてから branch を切る。release skill では develop/main にローカル独自コミットは想定しない。
...
- [ ] **Step 7. Wait CI & merge (release → develop)** — `gh pr checks <PR#> --watch && gh pr merge <PR#> --merge`
  - 確認: CI green を確認したら自動で merge。merged 状態を `gh pr view <PR#>` で確認してからチェック。CI 失敗時はユーザーに報告して停止。
...
- [ ] **Step 13. Sync local develop & main to HEAD** — `git fetch origin && git checkout main && git pull --ff-only origin main && git checkout develop && git pull --ff-only origin develop`
  - `pull --ff-only` が失敗した場合（local が diverged）は `git reset --hard origin/<branch>` でリセット。release skill では develop/main にローカル独自コミットは想定しない。
  - 確認: `git log develop..origin/main` と `git log main..origin/develop` が空であることを確認してからチェック。
...
- [ ] **Step 15. Start next feature branch** — `/create-feature-branch` を明示的に呼び出し、次の開発ブランチを作成する
  - release skill 内で完結させず、必ず `/create-feature-branch` skill に委譲すること（ブランチ命名規則と Git Flow の一貫性を保つため）。
  - **デフォルト動作**: ユーザーから明示的な指定（issue 番号・feature 名など）がない場合は、**次パッチバージョンの release ブランチを `develop` から派生**させる。例: 直前のリリースが `vX.Y.Z` なら `release/vX.Y.(Z+1)` を `develop` から切る。
  - 確認: 新ブランチが `git branch --show-current` で表示され、`develop` から派生していることを確認してからチェック。
